...
#
#   bash instalar.sh              # ambiente local: backend (venv+deps) + web (npm)
#   bash instalar.sh --iphone     # cadeia completa do iPhone (build+sync+pod+Xcode)
#   bash instalar.sh --tudo       # local + iPhone
#
...
if [ "$MODO" != "iphone" ]; then
  ...
  say "Prova de instalação: manifesto de arquivos"
  if [ -f scripts/verificar-arquivos.sh ]; then
    bash scripts/verificar-arquivos.sh || die "manifesto incompleto — o repositório não tem tudo que o app precisa"
  fi
...
echo "  próximo passo:  bash executar.sh        (rodar local)"
echo "                  bash atualizar.sh ...   (aplicar entrega e/ou deploy)"
