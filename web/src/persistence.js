...
function serverStore() {
  ...
  return {
    ...
    putAgent: (b) => sync.mutate("putAgent", [b], (cur) => ({ ...cur, agent: { ...(cur.agent || {}), ...b } })),
    cycle: () => api.cycle(),
...
function deviceStore() {
  ...
  return {
    ...
    async analyze(t) {
      ...
      const r = await api.analyzeTechnical(t, { config: doc.config, skill: doc.skill, profile: doc.profile, account, model: (arguments[1] && arguments[1].model) || "completo" });
    ...
    async buy(t, qty) {
      ...
      if (ex) {
        ...
        doc.positions.push({ t, qty, avg: +price.toFixed(2), stop: null, alvo: null });
      }
      doc.cash = +(doc.cash - qty * price).toFixed(2);
      doc.history.unshift({ date: now(), type: "COMPRA", t, qty, price: +price.toFixed(2), pnl: null });
      write();
      const out = pub();
    ...
    async sell(t) {
      ...
      const price = await priceOf(t);
      const pnl = +((price - pos.avg) * pos.qty).toFixed(2);
      doc.positions = doc.positions.filter((p) => p.t !== t);
      doc.cash = +(doc.cash + pos.qty * price).toFixed(2);
      doc.history.unshift({ date: now(), type: "VENDA", t, qty: pos.qty, price: +price.toFixed(2), pnl });
      write();
      const out = pub();
      out.priceUsed = +price.toFixed(2);
      return out;
