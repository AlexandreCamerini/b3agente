...
#
#   bash scripts/atualizar.sh ~/Downloads/files.zip "feat: pipeline IA 3 niveis"
#   bash scripts/atualizar.sh ~/Downloads/files.zip --somente-aplicar   # aplica+valida, sem git/deploy
#   bash scripts/atualizar.sh --somente-deploy "msg"                    # sem zip: só commit+push+health
#
...
#      /api/scan) e verifica também os endpoints NOVOS (/api/scan/deep/estimate).
set -uo pipefail
...
if [ -n "$ZIP" ]; then
  ...
  if [ -x "server/.venv/bin/python" ]; then
    ...
    echo "  (venv ausente — pulei as suítes; rode 'bash scripts/instalar.sh' para tê-las aqui)"
  fi
fi

[ "$SOMENTE_APLICAR" = "1" ] && { say "Concluído (somente aplicar)"; echo "  deploy quando quiser:  bash scripts/atualizar.sh --somente-deploy \"$MSG\""; exit 0; }
...
echo "  iPhone (quando houver mudança de app/plugin):  bash scripts/instalar.sh --iphone"
