...
REQ_SERVER=(
  ...
  server/app/agent.py server/app/push.py
...
# FASE 1 (Revisão Total): gates de qualidade nominais — o aceite da fase é código versionado
for f in server/tests/test_snapshot_consistency.py server/tests/test_setups_br.py qa/SETUPS.md; do
  [ -f "$f" ] && ok "$f" || falta "$f"
done
# FASE 2 (Revisão Total): funil Acompanhar→Radar→Watchlist→Portfólio + venda parcial + meta de entrada
for f in server/tests/test_fase2_portfolio.py web/tests/test_fase2_portfolio.mjs qa/13-fase2-funil.md; do
  [ -f "$f" ] && ok "$f" || falta "$f"
done

# FASE 3 (Revisão Total): cards (mock v2 aprovado) + Operador IA instrumentado (fix do timeout)
for f in server/tests/test_fase3_operador.py qa/14-fase3-cards-operador.md; do
  [ -f "$f" ] && ok "$f" || falta "$f"
done
...
for pat in "server/.venv/" "server/data/" "*.db" "*.p8" "node_modules/" "web/dist/" "web/ios/" ".env"; do
