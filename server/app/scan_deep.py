...
def _key(item: dict, period: str, modo: str = None) -> tuple:
    ...
    return (item["ticker"], period, item.get("snapshotId") or _day(),
            "operador" if modo == "operador" else "estudo")
...
def estimate(scan_payload: dict, top_n: int, period: str, modo: str = None) -> dict:
    ...
    cached = [r["ticker"] for r in sel if _key(r, period, modo) in _DEEP_CACHE]
    novas = [r["ticker"] for r in sel if _key(r, period, modo) not in _DEEP_CACHE]
    return {
...
async def run_deep(scan_payload: dict, top_n: int, period: str, deep_call, concurrency: int = 2, modo: str = None) -> dict:
