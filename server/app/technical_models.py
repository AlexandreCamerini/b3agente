...
def refocus(context: dict, model: str) -> dict:
    """FASE 1 (STU): o snapshot nasce com model='completo'; quando o usuário
    escolhe outro modelo no N2, este helper re-aponta os campos dependentes do
    modelo (labels + bloco `focus`) SEM recalcular nada — mesma tabela do
    build_context, fonte única."""
    model = normalize_model(model)
    context["model"] = model
    context["modelLabel"] = MODELS[model]["label"]
    context["modelDescription"] = MODELS[model]["description"]
    last = context.get("lastCandle") or {}
    context["focus"] = {
        "tendencia": context.get("trend"),
        "price_action": {"lastCandle": last, "levels": context.get("levels"), "volatility": context.get("volatility")},
        "momentum": context.get("momentum"),
        "volume": context.get("volume"),
        "volatilidade": context.get("volatility"),
        "suporte_resistencia": context.get("levels"),
        "swing_trade": {"trend": context.get("trend"), "levels": context.get("levels"), "riskPlanReference": context.get("riskPlanReference"), "volatility": context.get("volatility")},
        "opcoes": {"options": context.get("options"), "underlyingTrend": context.get("trend"), "underlyingVolatility": context.get("volatility")},
        "completo": "Usar todos os blocos.",
    }.get(model)
    return context
