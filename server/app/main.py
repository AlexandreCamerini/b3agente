...
SERVER_BUILD_ID = "F10-20260914-02"  # 2026-09-14: deploy so-backend (quick-260914-b6p). Corrige options_provider_mydata.get_options escolhendo vencimento por data FUTURA (dt.date, hoje injetavel) em vez de confiar no campo vence_no_pregao, que vem 0/falsy em 100% dos itens reais do mydata (medido ao vivo via railway run: BBAS3 27 vencimentos, PETR4 31, todos vence_no_pregao=0). Sem a correcao, numa segunda-feira o snapshot D+1 do mydata fazia o codigo escolher um vencimento ja passado, zerando tanto a proposta unica de venda coberta (Fase 14) quanto a curadoria nova (Fase 30) mesmo com estrutura liquida disponivel na mesma resposta. Achado nao implementado: o primeiro vencimento futuro pode cair abaixo do piso de 15 dias (BBAS3 mediu 4 dias no momento da medicao) - a cadeia default segue vazia por parte do ciclo mensal, agora por motivo legitimo. Suite 2864 passed (+8), 147 .mjs, exit 0. Front intocado (F10-20260914-01 continua sendo o BUILD_ID do web).
