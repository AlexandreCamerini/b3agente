...
SERVER_BUILD_ID = "F10-20260914-01"  # 2026-09-14: publicacao conjunta das Fases 27-30 (represada ate agora; nenhuma tinha ido ao ar). Fase 27: aba Opcoes passa a operar sobre a CARTEIRA (vigias por usuario, leitura tecnica interna custo zero, Emendas 1/2 ao ADR-027). Fase 28: PropostaLastreada extraida para modulo compartilhado (Emenda 3), sub-abas Setups/Operar, card de proposta removido do AtivoCard. Fase 29: flag opt-in `permitirOpcaoADescoberto` gateia `buy_option`/`sell_option` (a seco) no backend E no deviceStore (fechou bypass do iPhone offline); fechar posicao a seco nunca e bloqueado; termo de responsabilidade espelha o do Modo Operador. Fase 30: bloco "4 melhores vendas cobertas" em Posicoes — ranking 100% deterministico (premio/perda maxima via opcoes_motor.rastrear/avaliar), IA so narra o top ja ordenado (exigir_ranking recusa qualquer pool nao rankeado), custo zero de MCP (cadeia vem do options_provider mydata/Yahoo, nunca do mcp.semente.dev), narracao consome a mesma cota de /api/analyze. Suite 2856 passed, 5 skipped, 3 xfailed + 147 .mjs, exit 0.
