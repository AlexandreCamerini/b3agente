...
async def technicals(ticker: str, period: Optional[str] = None, scope: Optional[str] = Depends(current_scope)):
    ...
    if len(t) < 4:
        raise HTTPException(400, "Ticker invalido.")
    keep_req = candles_mod.resolve_keep(period)            # Objetivo 4: janela configurável
    ck = t + ":" + str(keep_req)
    hit = _TECH_CACHE.get(ck)
...
@app.get("/api/scan")
async def scan(period: Optional[str] = None, tickers: Optional[str] = None, scope: Optional[str] = Depends(current_scope)):
