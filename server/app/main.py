...
def _gate_analise(scope, config, custo: int = 1):
    ...
    2026-09-12: a precedência ganhou um degrau ACIMA dos dois — a CHAVE
    PRÓPRIA (BYOK). Continua sendo um único ponto de decisão; o que mudou é
    QUANDO o gate mensal é consultado, nunca O QUE ele mede (nenhum contador
    novo, `plan.py` intacto). Ver o porquê literal no corpo."""
    # BYOK vem ANTES do gate mensal, e a razão é literal: `month_used` conta
    # SÓ o consumo da chave do SERVIDOR (`metering.consume` é o único ponto de
    # escrita do ledger mensal e só roda no ramo gerenciado de
    # `_ai_apply_managed`), enquanto `plan.can_analyze` não sabe que existe
    # chave própria. Na ordem antiga, quem gastava as 30 análises gerenciadas
    # do mês e depois trazia a própria chave continuava barrado — por um
    # contador de um recurso que ele não ia usar, pagando do bolso pelo que
    # não conseguia rodar. O código já prometia o contrário em dois lugares
    # ("BYOK utilizável → sem cota" em `_ai_apply_managed`, e o texto do 402
    # falando de "análises do seu plano" para quem não usa a chave do plano);
    # isto faz a promessa valer (achado 2026-09-12, bloqueio real em conta).
    #
    # Mesma definição de "tem chave" que `_ai_apply_managed` usa — duas
    # definições de BYOK divergem na primeira manutenção. E `_ai_apply_managed`
    # com BYOK não tem efeito colateral nenhum: devolve `(config, lambda:
    # None)` na PRIMEIRA linha, sem tocar em `metering`.
    #
    # Sem BYOK nada muda: o gate mensal continua na frente do caminho
    # gerenciado, que é o que protege a chave do servidor (guardião em
    # tests/test_plan_gate_byok.py, caso 2).
...
SERVER_BUILD_ID = "F10-20260925-02"  # 2026-09-25: Fase 41 (Consolidacao de registros de tela, TELAS-01, milestone v1.8 Didatica ampliada + continuidade da aba Opcoes) publicada. Front: registro unico `web/src/telas.js` com as 8 telas do assistente centraliza a LISTA de onde cada tela aparece (barra, tour, ajuda, snapshot do assistente) - D-01; `BottomNav.defs` vira `defsDaBarra(cp)`, `petTela` vira `telaDoAssistente(tab, carteiraView)`, `tourPassos`/`ajudaSecoes` iteram `telasDoTour()`/`telasDaAjuda()` - zero lista paralela de tela restante em App.jsx (SC#1). Textos do tour/ajuda continuam num objeto local `porTela` dentro de cada funcao consumidora, nao no registro - D-02 (a LISTA centraliza, o CONTEUDO nao muda de lugar). Switch de `petSnapshot` intocado, amarrado ao registro por sha256 de bloco de codigo (mesma tecnica do fixture) - D-03. Refactor puro: equivalencia provada por fixture pre-refactor congelada (`web/tests/fixtures/telas_baseline_41.json`, verdade congelada que NAO se regenera) e confirmada ao vivo pelo Alex no checkpoint desta fase - D-04/D-05, zero mudanca visivel (rotulos, ordem, passos do tour, secoes da ajuda, resposta do assistente). Paridade com `conceitos.PET_TELAS` testada em dois pontos independentes (`test_telas_registro.mjs` + `server/tests/test_telas_paridade.py`), mesmo padrao de `defaults.py` x `catalog.js` - sem cruzar JS/Python num teste so. 6 guardioes antigos reconciliados com nota datada "REVERSAO DELIBERADA (2026-09-25, Fase 41, TELAS-01)" (5 previstos + `test_fase22_componentes_compartilhados.mjs`, achado na suite canonica), zero cobertura perdida. SC#4 demonstrado por walkthrough real (tela hipotetica "estudos": 16 assercoes JS + 1 pytest reprovando nomeando a causa, revertido, `git status --porcelain` limpo) - leitura aceita pelo Alex por nome no checkpoint: a LISTA de telas mora num ponto so e os testes passam a exigir/apontar cada peca que falta, mas o conteudo (textos, icone, snapshot) continua nos seus proprios lugares (D-02); nao virou "1 arquivo por tela nova" - isso nunca foi realista. Achados de inconsistencia registrados e NAO corrigidos nesta fase (`ajudaSecoes` com 11 secoes, so 6 mapeando 1:1 com uma tela, `historico`/`perfil` sem secao propria de ajuda; `tourPassos` cobre so 4 das 8 telas; rotulo do Radar diverge entre a barra - "Radar"/"Mesa", curto - e tour/ajuda - "Radar de mercado"/"Mesa de oportunidades", longo; "Operador IA" literal na Ajuda em vez de ler `cp.tituloOperadorIA`; "Acompanhar" literal nos tres lugares sem chave de `copy.js`; `NavIcon.paths` sem teste estrutural generico) - ciencia confirmada pelo Alex por nome no checkpoint desta fase ("aprovado, ciente dos achados, SC#4 ok", 2026-09-25), decisao de deixar para depois, sem plano de gap. Suite canonica fora do sandbox (`bash scripts/executar.sh --testes`): 3050 pytest passed/5 skipped/3 xfailed/0 failed (identico ao baseline 41-01/41-02, zero regressao) + 165/165 `.mjs` OK. `npx vite build` limpo (mesmo aviso pre-existente de chunk >500kB, nao introduzido por esta fase). Pendencia registrada, nao resolvida aqui: app iOS carrega bundle local - o registro so chega ao iPhone num build novo de TestFlight; como nada visivel nem de contrato mudou, o bundle antigo segue identico para o usuario. HISTORICO (entrega anterior, F10-20260925-01): 2026-09-25: Fase 40 (Continuidade da aba Opcoes, milestone v1.8 Didatica ampliada + continuidade da aba Opcoes) publicada. Front: ticker + aba ativa da tela Opcoes lembrados em memoria de sessao via `opcoesMemoria`/`escopoOpcoes` em App.jsx (nunca nos stores - D-01, evita acionar a paridade obrigatoria deviceStore/serverStore por um requirement que nao pediu persistencia entre reloads); so ticker+aba sao lembrados, o resto (Comparar expandido, tese/vencimento/alvo/stop, sheet de Vigias) volta ao default a cada visita - D-02; deep-link de Posicoes ("N estruturas destacadas...") vence so na ABA (abre em Destacadas) e MANTEM o ticker lembrado - D-03/DP-1, aprovado pelo Alex ao vivo em 2026-09-25 respondendo por nome a pergunta "Quando voce toca a linha de estruturas em Posicoes, Opcoes abre em Destacadas mas mantem o ativo que voce tinha escolhido em Montar. Aprova, ou prefere que esse toque tambem limpe o ativo?" com "aprovado" (sem plano de gap); `_resetScopeState()` limpa a memoria e incrementa `escopoOpcoes` (key de remount de OpcoesScreen), isolando 100% entre contas - D-04/cenario C2; restauracao silenciosa via inicializador lazy, sem flash de "Oportunidades"/"Escolha um ativo" antes do estado lembrado - D-05. Logica pura extraida para `web/src/opcoes/memoriaOpcoes.js` (abaInicialOpcoes/tickerInicialOpcoes/memoriaOpcoes), testada sem DOM. Guardiao novo `test_opcoes_continuidade_ui.mjs` com prova negativa real (removendo cada mitigacao isoladamente, o guardiao reprova nomeadamente); 4 guardioes antigos reconciliados com nota datada (allowlist de rede, inicializador de ticker, key de escopo no render, excecao nomeada na regra NAV-05 para o write-back sem I/O). Checkpoint humano ao vivo do Alex em 2026-09-25: roteiro de 10 cenarios (A, A2, B, B2, C, D, E, F, G + Modo Estudo) do 40-UI-SPEC aprovado, incluindo o cenario C (troca de conta sem vazamento) e o de rede (voltar com ativo lembrado nao dispara /api/options/mcp/* nem leitura paga). Suite canonica fora do sandbox (bash scripts/executar.sh --testes): 3048 pytest passed/5 skipped/3 xfailed/0 failed, identico ao baseline do 40-01; 1 falha .mjs (test_ios_assets.mjs, ambiental e conhecida - web/ios/ gitignored), zero regressao nos guardioes do produto. Pendencia registrada, nao resolvida aqui: app iOS carrega bundle local - a continuidade da aba Opcoes so chega ao iPhone num build novo de TestFlight; nenhum backend mudou nesta fase, o bundle antigo continua funcionando sem a memoria de sessao. HISTORICO (entrega anterior, F10-20260924-01): 2026-09-24: Fase 39 (Reestruturacao de navegacao da aba Opcoes, milestone v1.8 Didatica ampliada + continuidade da aba Opcoes) publicada, junto com o deviation de rename Recomendadas->Destacadas. Front: navegacao da aba Opcoes vira 3 abas fixas (Oportunidades / Destacadas / Montar), estado unico `abaOpcoes` substitui `subaba`/`abaWorkspace` (D-01); `VigiasBadge`+`VigiasSheet` no cabecalho, sheet visivel nas 3 abas (D-07); a execucao da proposta do motor COM gate (ex-SubAbaOperar) dissolvida em `PropostaDoAtivo`, painel inline dentro do card de Oportunidades - Leitura B' (DR-1, aprovada pelo Alex ao vivo, divergente do texto literal de D-05: `opcoes_lastreadas` e `opcoes_curadoria` sao motores diferentes, D-12 proibe mexer em opcoes_lastreadas.py nesta fase); 3a aba renomeada ao vivo pelo Alex de "Recomendadas" para "Destacadas" (override de D-03, ambiguidade regulatoria CVM com o disclaimer "nada aqui e recomendacao") mostra so a execucao da curadoria (motor sem gate), com piso de 60% de chance estimada OTM (Black-Scholes), ordenacao por premio anualizado, rotulo "1a de N" no lugar do score bruto (D-14), 5 estados vazios em cascata (D-11); aba Montar recebe `SecaoSetups` (fork 2) e link inline "Ver outros vencimentos" para Comparar; ⓘ "saiba mais" por aba (D-13) substitui o link fixo do topo; aviso de custo do frescor movido para o ⓘ do rodape GLOBAL da tela (DR-2, aprovada pelo Alex, divergente do UI-SPEC que pedia o ⓘ de Montar - `mcpStatus` dispara no mount da tela inteira, aviso so em Montar deixaria quem fica em Oportunidades/Destacadas pagar sem ver o aviso, regressao da Fase 27-05); linha de chamada em Posicoes ("N estruturas destacadas...") abre Opcoes direto em Destacadas. Backend: `PISO_PROB_OTM=0.60`, `vol_do_contrato`/`prob_otm`/`premio_anualizado`/`aplicar_piso` em opcoes_curadoria.py, ordenacao por `premioAnualizado`, `_hv21_do_ativo` lazy 1x por ticker em main.py, meta com `pisoProbOtm`/`admitidosNoPiso`/`reprovadosNoPiso`/`semProbabilidade` (D-11); campo `razao` mantido no dict por compatibilidade (bundle iOS antigo continua funcionando). 10 guardioes de teste reconciliados com nota datada (39-05), zero cobertura liquida perdida. Checkpoint humano ao vivo aprovado pelo Alex: roteiro de 10 itens + DR-1 e DR-2 nomeadas e aprovadas explicitamente + 3 riscos respondidos (a: nome trocado para Destacadas; b: ciente, sem mudanca agora - put de protecao/collar perto do dinheiro quase nao entram na lista com o piso 60%; c: "mostrar 3 e deixar escolher mais" fica pra depois, nao vira requisito agora). Suite canonica fora do sandbox (bash scripts/executar.sh --testes): 3048 pytest passed/5 skipped/3 xfailed/0 failed + 163/163 .mjs, exit 0, identico a baseline do 39-05/deviation Destacadas. Pendencia registrada, nao resolvida aqui: app iOS carrega bundle local, a navegacao nova so chega ao iPhone num build novo de TestFlight - o backend (piso) ja vale para ele, o bundle antigo continua funcionando e ainda mostra "Pontuacao de curadoria" porque `razao` continua no dict, mas a lista que ele recebe ja e a filtrada/ordenada pela regra nova. HISTORICO (entrega anterior, F10-20260923-02): 2026-09-23: quick 260923-ndy (achado ao vivo do Alex: em Opcoes -> Analisar, montar uma estrutura manual nao tinha caminho de execucao - so a lista de curadoria "as 4 melhores" executava). Backend: `_execucao_da_proposta(estruturas, lote)` em options_mcp_api.py deriva tipo/contrato/contratos/qtyAcoes SEMPRE no servidor (principio 5 do CLAUDE.md, zero aritmetica financeira no front); campo `execucao` novo em POST /api/options/mcp/proposta. Mapa perna->tipo: sell+CALL->call_coberta, buy+PUT->put_protecao (exige lastro), buy+CALL->opcao_a_descoberto, sell+PUT->nao executavel (sem rota no simulador); 2+ pernas (trava/collar) -> MOTIVO_EXEC_MULTIPERNA, sem botao; lote precisa ser multiplo de 100 (1 contrato = 100 acoes) senao MOTIVO_EXEC_LOTE_CENTENA. Front: ExecutarProposta.jsx (novo) ligado em SecaoAnalisar.jsx, despacha pelo MESMO caminho ja em producao (ctx.A.executarCandidatoCurado -> executarCandidato.js -> store) - nenhuma rota nova, nenhuma logica de execucao duplicada. Gate de Modo Estudo (sem botao), motivo/erro do servidor verbatim, consentimento de liquidez DIFICIL por checkbox (so aparece apos a recusa "Liquidez DIFICIL (score..." do servidor). Risco de produto declarado, nao resolvido: a fixture real do servico MCP tende a propor travas de 2 pernas - se esse padrao se confirmar em producao, a MAIORIA das estruturas montadas em Analisar cai em "nao executavel aqui"; executar trava/collar do Analisar exigiria rota multi-perna nova (store.py so executa 1 perna por chamada), fora do escopo desta quick. Pre-existente nao corrigido aqui: /api/options/buy (usado por opcao_a_descoberto) nao tem o 403 de Modo Estudo que lastreada/abrir tem (mitigado no front pelo gate de UI). 16 testes novos (14 backend + 2 rota) + guardiao frontend com 35 assercoes + prova negativa real (gate removido -> 3 falhas nomeadas -> revertido -> verde). Suite reconciliada em 3 medicoes: baseline pre-mudanca 2964 passed/27 falhas conhecidas de TLS-sandbox -> pos-mudanca 2980 passed (mesmo sandbox) -> 3007 passed fora do sandbox, 0 regressao. Pendencia: app iOS carrega bundle local, botao so chega num build novo de TestFlight. HISTORICO (entrega anterior, F10-20260923-01): 2026-09-23: Fase 38 (KB Didatica ampliada, milestone v1.8 Didatica ampliada + continuidade da aba Opcoes) publicada. Backend: titulo autorado/derivado nos 83 verbetes da KB (65+18), FAMILIAS rotuladas, rota GET /api/kb/catalogo (38-01). Front: SetorAlvo/ConceitoSheet/AssistenteBox/AiNote extraidos verbatim para web/src/entendimento.jsx (38-02); ConceitoSheet ganha discriminador fonte='conceito'|'kb' (default conceito, sem fallback silencioso por 404), kbCatalogo nos dois stores/ctx (paridade), acao A.abrirVerbeteKb separada de A.abrirVerbete para nao tocar a assinatura que test_concentracao_carteira.mjs trava (38-03); tela Glossario em Perfil->Ajuda com busca live client-side por SUBSTRING (nao palavra inteira - D-02 trava so kb.buscar() do backend, esta tela nao o chama) + 9 familias em acordeao recolhido, tile com contagem dinamica de verbetes, estados vazio/erro honestos (KB-01, 38-04); link "saiba mais" fixo em Acompanhar/Radar/Watchlist (overlay global, ANCORAS_KB congelado em glossario.js) e em Opcoes (ConceitoSheet LOCAL com estado proprio, importada so de entendimento.jsx/glossario.js - preserva isolamento ADR-027, OpcoesScreen.jsx com ZERO import de App.jsx) - D-08 aprovado pelo Alex via AskUserQuestion sem alteracao da proposta original do 38-UI-SPEC.md (Acompanhar->mkt-carteira-simulada, Radar->confluencia, Watchlist->ind-rsi, Opcoes->mkt-opcao) (KB-02, 38-05). Verificacao humana ao vivo (38-06, roteiro de 9 itens) aprovada pelo Alex - achado positivo nao previsto pelo risco declarado no 38-05: o FAB do Boris fica escondido sob a folha local de Opcoes por z-index puro (overlay=86 > FAB=60, elementFromPoint confirma), a tensao de duas vias de folha na mesma tela nao se materializou como defeito visual/funcional; achado cosmetico menor, nao-bloqueante, registrado como pendencia de polish: dois icones "×" sobrepostos no campo de busca do Glossario em 375px (input type="search" nativo do WebKit/Chromium + "×" customizado do app). Suite fora do sandbox: 2964 pytest passed/5 skipped/3 xfailed (as 27 falhas vistas DENTRO do sandbox sao o artefato de rede/TLS pre-existente e documentado do projeto, nenhuma toca kb.py/App.jsx/OpcoesScreen.jsx/glossario.js/entendimento.jsx) + 159/160 .mjs (unica falha, test_ios_assets.mjs, e ambiental e conhecida - web/ios/ gitignored). npx vite build limpo. Pendencia registrada, nao resolvida aqui: build iOS/TestFlight ainda nao reflete esta entrega (app nativo carrega bundle local, sem server.url) - Glossario e os 4 "saiba mais" so chegam ao iPhone num build novo de TestFlight; a rota de backend ja vale para ele. HISTORICO (entrega anterior, F10-20260922-01): 2026-09-22: Fase 37 (Grafico de Payoff e Explicacao Confiaveis, milestone v1.7 Confiabilidade explicativa da aba Opcoes) publicada. SecaoAnalisar.jsx e SecaoComparar.jsx passam a passar dominio/segmentos/valorHoje ao PayoffChart corrigido (eixo Y com escala real e zero rotulado "R$ 0", strikes e spot marcados, seta de risco ilimitado rotulada "sem teto"/"sem piso", dominio sem cortar plato) e a renderizar ExplicacaoPayoff (explicacao por segmento 100% deterministica, reusando literalmente a mesma variavel de RazaoGanhoPerda, nunca recalculada - D-05/D-06). Bloco "Hoje - valor de mercado" aparece separado de "No vencimento" e, em Comparar, so no 1o candidato da lista - D-02 (sem gate por indice reimplementado no front, a fonte unica do gate e o backend). Rotulo "Razao" da curadoria renomeado para "Pontuacao de curadoria" - D-04. CuradoriaEstruturas.jsx NAO foi tocado nesta fase (D-09) - recebe a correcao de geometria de graca via fallback local em PayoffChart.jsx, sem os blocos novos. Checkpoint humano da Task 2 aprovado pelo Alex com ressalva registrada: a verificacao visual ao vivo com dado real de mercado NAO pode ser concluida porque o backend local usado na tentativa nao tinha BRAPI_TOKEN/BOLSAI_API_KEY configurados (aba Opcoes nao respondia nada em opcoes); aprovacao se apoiou na suite canonica verde (3x ao longo das ondas), no vite build verde e na verificacao de codigo linha a linha repetida pelo orquestrador - nao numa confirmacao visual do grafico/explicacao corrigidos com numeros reais em producao. Verificacao visual em producao apos esta publicacao fica registrada como pendencia recomendada, sem bloquear nada. Suite 2973 pytest passed + 156/156 .mjs, exit 0, identico a baseline. HISTORICO (entrega anterior, F10-20260921-01): 2026-09-21: Fase 35 (Jornada Guiada do Workspace, milestone v1.7 Confiabilidade explicativa da aba Opcoes) publicada. Workspace ganha dois estagios nomeados sem numeracao falsa: bloco de leitura paga anuncia "Passo 1 de 2", carril de pills anuncia "Passo 2 de 2" (Analisar/Comparar nunca numeradas entre si, principio 5 do CLAUDE.md — nao inventar sequencia que o produto nao tem). Bug corrigido: a pill "Setups salvos" nao depende da leitura paga mas era bloqueada pelo convite dela mesmo assim — convite some quando essa pill esta ativa. Apos a leitura, convite vira "leitura ja feita" e aparece linha de transicao acima do carril. Os 3 CTAs da jornada (Ler no servico de opcoes, Montar estrutura, Ver possibilidades) preenchidos com T.accent solido + texto T.onAccent — nunca #fff literal nem color-mix, que reprovavam contraste AA em ate 4 das 4 combinacoes tema x modo medidas; extensao aprovada pelo Alex do reserved-for de accent travado no UI-SPEC da Fase 34. Marca de resultado (check neutro, T.textMuted) aparece nos dois CTAs re-clicaveis quando ha resultado com conteudo, sem nunca desabilitar o botao — trocar tese/parametros e montar de novo continua funcionando. Fold-in real achado pela propria varredura de tokens desta fase: SecaoVigias.jsx referenciava T.bgPanel fora do array TOKENS desde a Fase 33 — chip de vigia nao selecionado renderizava sem fundo, em silencio, corrigido. Checkpoint humano do Alex aprovado AO VIVO (nao so por leitura de codigo), incluindo as 8 leituras de contraste nas 4 combinacoes tema x modo — fecha o risco aberto de D-07 com evidencia visual alem da tabela computada. Suite 2923 passed + 153/154 .mjs (so a falha ambiental pre-existente de test_ios_assets.mjs, web/ios/ gitignored), exit 0, identico a baseline. Pendencia registrada, nao resolvida aqui: build iOS/TestFlight ainda nao reflete esta entrega (app nativo carrega bundle local, sem server.url). HISTORICO (entrega anterior, F10-20260920-01): 2026-09-20: publicacao conjunta das Fases 33+34 (milestone v1.6, Simplificacao da aba Opcoes) — a Fase 33 fechou verificada (7/7) em 2026-09-16 e nunca tinha ido ao ar. Fase 33: os 5 jobs da sub-aba Setups (descobrir oportunidades, vigias, analisar ticker, comparar vencimentos, gerenciar setups) saem da rolagem unica e viram componentes proprios em web/src/opcoes/ (SecaoDescobrir/SecaoVigias/SecaoSetups/SecaoComparar/SecaoAnalisar), zero mudanca de comportamento/dado/ordem visivel. Fase 34: a sub-aba Setups troca a rolagem unica por dois modos sobre o estado `ticker` ja existente — hub (sem ticker: descoberta + vigias + seletor) e workspace (ticker selecionado: header fixo com nome+Voltar, leitura tecnica, pill row de 3 abas Analisar/Comparar/Setups salvos). Achado real do planejamento, verificado e confirmado pelo Alex: a leitura original da decisao D-01 pedia uma listagem de "setups salvos" cross-ticker no hub, mas isso e impossivel com o modelo de dado atual (`setups` deriva so de `leitura.dados`, a leitura PAGA ticker-scoped, sempre vazia com `ticker=""`) — resolvido com o hub usando a listagem de vigias (que ja e cross-ticker, custo zero) e `SecaoSetups` inteira (lista+criacao) migrando pro workspace como 3a aba. NAV-05 (trocar de aba nao dispara nova leitura paga) verificado ao vivo por contagem de rede antes/depois de cada troca de pill, nao so por leitura de codigo. Checkpoint humano do Alex aprovado antes desta publicacao. Suite 2923 passed + 152/153 .mjs (so a falha ambiental pre-existente de test_ios_assets.mjs, web/ios/ gitignored), exit 0, identico a baseline das duas fases. Pendencia registrada, nao resolvida aqui: build iOS/TestFlight ainda nao reflete esta entrega (app nativo carrega bundle local, sem server.url). HISTORICO (entrega anterior, F10-20260916-03): 2026-09-16: quick 260916-g6p — fecha a unica ressalva aberta da Fase 32 (deferred-items.md). Guard null-safe explicito substitui `|| 0` nas tres copias do helper `porLote`/CTA de collar (CuradoriaEstruturas.jsx, CandidatoOpcao.jsx, PropostaLastreada.jsx) — campo ausente agora renderiza "-" via price(null), nunca "R$ 0,00" (principio 4). Duas armadilhas medidas antes de editar: Math.abs(null) === 0 exige o guard ENVOLVENDO Math.abs, nao dentro dele; o CTA de collar alcanca o ramo debito tambem com p.caixa falsy, entao o `p.caixa &&` na frente ficou. `ARQUIVOS_EXCECAO_OU_ZERO` removida do guardiao OU_ZERO em test_opcoes_analisar_ui.mjs; PropostaLastreada.jsx (copia original, Fase 28, nunca esteve na allowlist) entra na regra pela primeira vez, com prova negativa real (defeito reintroduzido a mao, guardiao falhou nomeando o arquivo, revertido, guardiao voltou a passar). Suite 2923 passed + 152 .mjs, exit 0, identico a baseline da Fase 32. HISTORICO (entrega anterior, F10-20260916-02): 2026-09-16: quick 260916-cod — fecha os achados do code review da Fase 32. WR-01: a linha de chamada em Posicoes (e o bloco B da aba Opcoes) exibia "nenhuma oportunidade"/"nenhuma estrutura elegivel" no render ANTERIOR ao ligamento da flag monotonica `curadoriaAtiva` — afirmava resultado antes de a varredura comecar, a mesma classe de erro que a propria Fase 32 corrigiu no estado de ERRO (principio 4). `useCuradoria` passa a expor `concluido` (nasce false, vira true no finally da primeira busca, cobrindo sucesso e falha) e os DOIS call sites tratam `!concluido` como "ainda nao medi", antes do ramo de vazio. NAO foi reusado `carregando` para isso de proposito: ele significa "requisicao em voo", e sobrecarrega-lo reintroduziria o "Verificando..." eterno quando a flag nunca liga. Provado ao vivo no caminho frio: a sequencia agora e "Verificando oportunidades de opcoes..." -> "4 oportunidades...", nunca vazio precoce. Junto, higiene da extracao: imports orfaos PayoffChart/estruturaParaPayoff removidos de App.jsx e sombreamento de `item` desfeito em CuradoriaEstruturas.jsx. Fechada tambem a divida de verificacao do item 8 da Fase 32 (ramo multi-candidato visto renderizando com ABEV3: put de protecao + trava protetora lado a lado, mesma largura) — sem alterar codigo para produzir o cenario: a coexistencia exige plano de VENDER/baixa (opcoes_lastreadas.py:247), entao a fixture foi escolher a posicao certa. Suite 2923 passed + 152 .mjs, exit 0. HISTORICO (entrega anterior, F10-20260916-01): Fase 32 — consolidacao das operacoes de opcoes na aba Opcoes. Os dois blocos cross-carteira (proposta confirmada pela leitura tecnica + as 4 melhores da varredura) saem de Posicoes e passam a viver lado a lado na aba Opcoes, sob uma frase-ponte que nega hierarquia entre os dois motores (D-05, mitigacao regulatoria); Posicoes fica com UMA linha de chamada cuja contagem le a fonte unica `ctx.curadoria` (D-03) e que NUNCA mostra numero no estado de erro. Correcao de principio 4 junto: a curadoria exibia "nenhuma estrutura elegivel" tambem quando a BUSCA FALHAVA — erro agora tem precedencia sobre vazio, com texto e CTA proprios. `OportunidadesOpcoes`/`CuradoriaEstruturas`/`CandidatoOpcao`/`useOpcoesPropostas` extraidos de App.jsx para web/src/opcoes/; `PropostaDaPosicao` morreu e a sub-aba Operar ganhou o ramo multi-candidato (D-04). Cadeia de execucao do collar curado INTOCADA (auditoria 32-05 + verificacao ao vivo: venda coberta por clique real, collar pelo mesmo `executarCandidato`+store). Suite 2923 passed + 152 .mjs, exit 0. HISTORICO (entrega anterior, F10-20260915-02): quick 260915-ndt fez o collar da lista curada executar via rota nova `POST /api/options/curadoria/abrir-collar`, que re-deriva pelo motor da curadoria em vez de `opcoes_lastreadas.propor()`. Causa do 409: `lastreada/abrir-collar` re-derivava por `opcoes_lastreadas.propor()` (que EXIGE endosso da leitura tecnica) enquanto o card vinha de `opcoes_curadoria` (que deliberadamente NAO tem esse gate, ver opcoes_curadoria.py:211-212) — todo collar curado era rejeitado sempre que a leitura tecnica nao endossava. Rota NOVA `POST /api/options/curadoria/abrir-collar` re-deriva pelo motor da curadoria via `_curadoria_scan_posicao` (mesmo helper do `_curadoria_top`, uma varredura so). Defesa anti-adulteracao INTACTA: strike/expiration/ivEntrada/premios saem do candidato re-derivado e da cadeia, nunca do corpo; `idCandidato` e so chave de busca; `permitir_a_descoberto=False` hardcoded. Rota antiga, `lastreada/abrir` e `options/buy` intocadas. Junto: rotulo `curadoriaPremioRotulo` proprio para o premio no painel inline (antes usava o rotulo da RAZAO exibindo valor em reais). Suite 2923 passed (+13), 150 .mjs, exit 0. Front nesta mesma entrega.
