...
def test_propor_collar_canonico_liquidez_eh_a_menor_das_duas_pernas():
    # ATUALIZADO 2026-09-08 (quick 260908-dnl): a fixture era call 1000/1000
    # e put 100/100. Com `liquidity_score` recalibrado (OI substitui volume em
    ...
    r = opcoes_lastreadas.propor("PETR4", _cadeia(calls=calls, puts=puts), _SPOT, _PLANO_VENDER,
                                  _posicao(qty=300), 50, "operador", _HOJE, multiperna=True)
    liq_call = liquidity_score(10000, 10000, calls[0]["bid"], calls[0]["ask"])
    liq_put = liquidity_score(2000, 2000, puts[0]["bid"], puts[0]["ask"])
