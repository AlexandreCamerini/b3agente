...
# ===== BLOCO B1 — progresso observável =====
def test_progresso_chega_ao_total_e_conclui():
    import asyncio
    from app import candle_cache, scanner

    async def fetch(symbol, rng):
        return {"candles": [{"date": f"2026-01-{i+1:02d}", "open": 10, "high": 10.5, "low": 9.5, "close": 10.2, "volume": 100} for i in range(60)], "currency": "BRL"}
    scanner._SCAN_CACHE.clear()
    candle_cache._CACHE.clear()      # isola dos demais testes (cache é global)
    asyncio.run(scanner.run_scan(period="6mo", universe="PPPP3,QQQQ4,RRRR3", fetch=fetch))
    candle_cache._CACHE.clear()      # não polui quem vem depois
    p = scanner.SCAN_PROGRESS
    assert p["feitos"] == p["total"] == 3
    assert p["ativo"] is False and p["fase"] == "concluído"
    assert len(p["ultimos"]) <= 3
