...
Sob o sandbox padrão do Bash tool, 27 testes pré-existentes e não relacionados a este plano falharam com `PermissionError: [Errno 1] Operation not permitted` ao carregar certificados TLS (`ssl.load_verify_locations`) — módulos afetados: `test_benchmark_ibov.py`, `test_yahoo_*.py`, `test_push_registro_evento.py`, `test_texto_vazio.py`, `test_opcoes_lastreadas_rotas.py`, `test_rotas_fase4.py`, `test_fase3_kill_switch_duracao.py`, `test_options_provider_yahoo.py`. Nenhum desses testes toca `opcoes_gatilho.py`/`setups.py`/`opcoes_lastreadas.py`. Confirmei que é artefato de sandbox (não regressão) rodando a suíte completa uma vez com `dangerouslyDisableSandbox: true`: **1832 passed, 1 skipped** (incluindo os 16 testes novos deste plano). Fora de escopo deste plano por desenho (SCOPE BOUNDARY) — não fiz nenhuma alteração para "corrigir" isso.
