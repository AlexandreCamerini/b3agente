...
- [x] **STORY-01**: Documentar a jornada real do usuário do primeiro acesso
...
- [x] **STORY-02**: Avaliar se a transição Estudo→Operador tem gatilho e
      narrativa claros (o que muda, por que, quando o usuário deveria migrar)
      ou é apenas um toggle sem contexto
- [x] **STORY-03**: Avaliar cobertura real da camada educacional (tendência,
...
- [x] **STORY-04**: Verificar que a IA nunca promete lucro, nunca afirma
...
- [x] **UX-01**: Auditar os 10 princípios obrigatórios do CLAUDE.md contra
...
- [x] **CODE-03**: Avaliar o defeito já documentado do gate "Executar" (trava
      ...
      radius
- [x] **CODE-04**: Avaliar cobertura da suíte canônica
...
- [x] **GATE-01**: Avaliar se `plan.py`/`metering.py` aguentam escalonamento
...
- [x] **GATE-02**: Avaliar a separação entre cota física da brapi
      (compartilhada, 15k/mês) e cap comercial (por conta) — a UI comunica
      essa distinção de forma transparente quando o cap disparar?
- [x] **GATE-03**: Mapear tecnicamente quais features hoje são candidatas
...
- [x] **ADMIN-01**: Avaliar cobertura do portal (`web-admin/`) contra RBAC
      (ADR-013) — os grupos/papéis cobrem os cenários reais de operação?
- [x] **ADMIN-02**: Avaliar visibilidade operacional (kill-switch, orçamento
      brapi, métricas de IA) — o portal teria detectado e permitido agir
      rápido num incidente como o do kill-switch de 2,5 dias?
- [x] **ADMIN-03**: Avaliar usabilidade do handoff mobile (ADR-014) e do
      fluxo geral de quem administra o produto

### Consolidação

- [x] **REPORT-01**: Relatório único consolidando todos os achados acima,
