...
Last activity: 2026-09-10 — quick 260910-biz: **Aba Opções, Fase 2 de 6** mesclada na branch. A barra inferior agora mostra Opções no lugar de Operador IA, que passou a ser sub-tela do Portfólio. Front novo isolado em `web/src/opcoes/`. Fase 1 (ADR-027, cliente MCP, cap) já está em `main` (PR #38, 7bc7143). Próximo: Fase 3 (cadeia, catálogo, proposta e possibilidades por vencimento) após revisão do Alex. Pendências do Alex: `MCP_CLIENT_ID`/`MCP_CLIENT_SECRET` no Railway de staging (sem eles a aba mostra "serviço não configurado" e nada foi verificado ao vivo); avalizar (a) chamada que morre em erro de tool não consome cap e (b) `/setups/{name}/grafico` sem `frescor`. Produção segue em F10-20260908-02.
