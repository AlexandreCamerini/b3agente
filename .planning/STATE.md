...
Last activity: 2026-09-10 — aba Opções **funcionando em produção** (leitura de PETR4/VALE3 confirmada pelo Alex no app; log mostra `propose_option_setups ms=6167`, `list_setups cache=True`). Corrigido na sequência o terceiro defeito da MESMA classe achado ao vivo (quick 260910-red): frescor escolhido por origem da chamada em vez de qualidade da medição, fazendo a aba dizer "não medido" com dado em dia — **pendente de publicação** (`bump.sh` + `publicar-web.sh` + clique no Railway). Produção hoje em `F10-20260910-01`. Aberto da auditoria (`docs/AUDITORIA-2026-09-10.md`): lote MCP A-01 a A-05 (dois deles afirmam o que não mediram), A-07 (cota de IA estoura sob concorrência), A-08 (agente anuncia venda que não houve), A-17/A-18 (guardiões provados cegos), e o ValueError de `/api/options/buy`. A-06 (timeout) perdeu urgência: em produção a chamada leva 1-6 s, não os 8,2 s medidos localmente.
