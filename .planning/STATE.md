...
Last activity: 2026-09-10 — lote MCP da auditoria fechado (quick 260910-svy, A-01 a A-05): a aba deixa de afirmar teto sem medição, o log passa a dizer o motivo real, as versões do SDK deixam de ser decididas pela data do deploy, e token recusado não vira indisponibilidade de uma hora. **Deploy disto é SÓ-BACKEND e exige bump manual do `SERVER_BUILD_ID`** — ainda NÃO feito. Produção em `F10-20260910-02` (aba Opções funcionando, leitura de PETR4 confirmada pelo Alex). Nota de ambiente: `test_ios_assets.mjs` falha quando `web/dist` é rebuildado sem `cap sync` depois — artefatos locais dessincronizados, não regressão; `npx cap sync ios` resolve. Aberto da auditoria: A-07 (cota de IA estoura sob concorrência), A-08 (agente anuncia venda que não houve), A-17/A-18 (guardiões provados cegos), A-09/A-10/A-14/A-15/A-16 (baixos), e o ValueError de `/api/options/buy`. A-06 (timeout) fechado sem ação: produção leva 1-6 s, não os 8,2 s locais.
