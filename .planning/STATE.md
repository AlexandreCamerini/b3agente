...
Last activity: 2026-09-10 — **auditoria completa do projeto** (`docs/AUDITORIA-2026-09-10.md`, 21 achados, 4 finders + verificação adversarial) e correção do achado mais grave dela: `/api/buy` aceitava quantidade negativa e executava a ordem (quick 260910-mqs, `45fdbe3`). Três achados de concorrência na carteira foram REFUTADOS por reprodução. Pendentes de decisão: lote do MCP (A-01 a A-06, dois deles fazem o app afirmar o que não mediu), cota de IA que estoura sob concorrência (A-07), agente anunciando venda que não houve (A-08), e dois guardiões provados cegos (A-17, A-18). A aba de Opções SEGUE SEM FUNCIONAR em produção — credencial e cliente provados bons fora do Railway; falta ler `railway logs | grep '[b3][mcp]'`, que separa credencial recusada de indisponibilidade. PR #39 aberto acumulando Fase 2 + conserto do cabeçalho + esta correção.
