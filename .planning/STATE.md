...
Last activity: 2026-09-09 — quick 260909-tmi: `boris.semente.dev` fechado como único domínio canônico na doc viva/comentários/fixtures (código já era). Antes: 260909-th4 (`SERVER_BUILD_ID` → `F10-20260909-01`, PR #37 aberto) e 260909-oyu (carimbo das ordens em BRT, PR #36 mesclado). **Deploy de produção pendente de clique do Alex no Railway**; confirmação = `curl -s https://boris.semente.dev/api/health` responder `F10-20260909-01`. Upload do build 10 ao TestFlight ainda sem confirmação.
