...
Last activity: 2026-09-10 — quick 260910-d57: consertados dois defeitos do cabeçalho da aba Opções achados no **teste ao vivo no iPhone** (app instalado da branch contra produção). O sintoma "frescor não medido" era o default da tela, não uma medição: produção só tem a Fase 1, `/leitura` responde 404. Vai junto no PR #39 (Fase 2). Produção confirmada em F10-20260909-01 (deploy da Fase 1 feito). Pendências do Alex: mesclar #39, deployar, e pôr `MCP_CLIENT_ID`/`MCP_CLIENT_SECRET` no Railway — sem o segredo a aba diz "serviço não configurado" mesmo com tudo no ar. Próximo: Fase 3 (cadeia, catálogo, proposta e possibilidades por vencimento).
