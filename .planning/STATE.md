...
- **Fase 40 (ESTADO-01, ex-Fase 39) herda `entendimento.jsx`/`glossario.js` como módulos de terceiros já disponíveis** — qualquer trabalho de continuidade de estado da aba Opções pode importar deles sem recriar nada; nenhuma mudança de contrato é esperada desses dois módulos vinda da Fase 40. Depende também da navegação de 3 abas da Fase 39 (NAV-01) já estar definida.

**Stopped at:** Phase 39 UI-SPEC aprovado — `/gsd-ui-phase 39` completo em 2026-09-24. `39-CONTEXT.md` (14 decisões, commit `2759fae`) → gate de UI bloqueou o primeiro `/gsd-plan-phase 39` (fase tem `UI hint: yes`, sem UI-SPEC) → `gsd-ui-researcher` gerou `39-UI-SPEC.md` (commit `8cd6179`, reusa tokens `T.*` de `32/34/38-UI-SPEC.md`, zero perguntas ao usuário — CONTEXT.md já respondia tudo) → `gsd-ui-checker` 1ª rodada: BLOCKED (Dimension 4 Typography — 6 tamanhos de fonte declarados, máximo é 4, sem exceção citada) + 1 flag não-bloqueante (Dimension 2 Visuals — sem foco visual primário por aba) → revisão (`gsd-ui-researcher`, commit `3fb61f6`): tamanhos consolidados em 4 papéis (Heading/Eyebrow/Body padrão/Body pequeno, mesma taxonomia de `32-UI-SPEC.md:74-79`) + seção "Foco visual primário por aba" adicionada → `gsd-ui-checker` 2ª rodada: APPROVED 6/6 (citações spot-checked contra código real: `CuradoriaEstruturas.jsx:194/268`, `OpcoesScreen.jsx:140-144/540-550`, `copy.js`). Sign-off e frontmatter (`status: approved`) atualizados à mão (commit `a8f56b8`) — `state.record-session` do gsd-sdk não foi chamado, ver [[gsd-sdk-state-corrompe]].

**Decisões do UI-SPEC que o próximo leitor não deve redescobrir (NAV-01):** correção obrigatória de débito pré-existente `#fff`→`T.onAccent` em `CuradoriaEstruturas.jsx:268` entra no escopo da fase (Dimension 3 do checker a exigiu). 3 Open Questions ficam como flag para o planner/checkpoint, não bloqueiam o UI-SPEC: (1) destino do clique num card de Oportunidades após "Operar" sumir — recomendação do UI-SPEC é navegar para Montar (Leitura A), alternativa registrada é portar `handleExecutar` para `OportunidadesOpcoes.jsx` (Leitura B); (2) onde `SecaoSetups.jsx` mora dentro de "Montar" — proposta: seção sem gate abaixo do link de comparação, sem aba própria (D-01 proíbe 2ª camada); (3) a frase-ponte regulatória da Fase 32 ("nenhuma é mais certa que a outra") precisa sobreviver na reescrita de `curadoriaSubtitulo` quando Oportunidades/Recomendadas viram abas separadas — texto final é território do plano, não do UI-SPEC. Estado de erro não-tratado de Oportunidades (débito herdado de `32-UI-SPEC.md` Open Question #3) segue fora de escopo do NAV-01, declarado explicitamente, não escondido.

**Próximo passo:** `/gsd-plan-phase 39` (gate de UI-SPEC já satisfeito, `39-UI-SPEC.md` aprovado no diretório da fase) — resume file: `.planning/phases/39-reestrutura-o-de-navega-o-da-aba-op-es/39-UI-SPEC.md`.
...
| 260923-ndy | **Achado ao vivo do Alex**: em Opções → Analisar, montar uma estrutura manual ("Montar estrutura" → `montarProposta()`, motor MCP externo `mcp.semente.dev`) é só leitura — nenhum caminho de execução, diferente da lista de curadoria "as 4 melhores" que já executa. Se o dono do produto não consegue efetivar, o usuário final também não vai. Backend: `_execucao_da_proposta(estruturas, lote)` em `options_mcp_api.py` deriva tipo/contrato/contratos/qtyAcoes SEMPRE no servidor (princípio 5 do CLAUDE.md — zero aritmética financeira no front); campo `execucao` novo em `POST /api/options/mcp/proposta`. Mapa perna→tipo: sell+CALL→`call_coberta`, buy+PUT→`put_protecao` (exige lastro), buy+CALL→`opcao_a_descoberto`, sell+PUT→não executável (sem rota no simulador). Frontend: `ExecutarProposta.jsx` (novo) ligado em `SecaoAnalisar.jsx`, despacha pelo MESMO caminho já em produção (`ctx.A.executarCandidatoCurado`→`executarCandidato.js`→store) — nenhuma rota nova, nenhuma lógica de execução duplicada. Gate de Modo Estudo, motivo/erro do servidor verbatim, consentimento de liquidez DIFÍCIL por checkbox. **Risco de produto declarado para o Alex decidir**: a fixture real do serviço MCP tende a propor travas de 2 pernas — se esse padrão se confirmar em produção, a MAIORIA das estruturas montadas cai em "não executável aqui" (`MOTIVO_EXEC_MULTIPERNA`); executar trava/collar do Analisar exigiria rota multi-perna nova (`store.py` só executa 1 perna por chamada), fora do escopo desta quick. Pré-existente não corrigido aqui: `/api/options/buy` (usado por `opcao_a_descoberto`) não tem o 403 de Modo Estudo que `lastreada/abrir` tem (mitigado no front pelo gate de UI, defesa de servidor mais fraca nesse caminho específico). 16 testes novos (14 backend + 2 rota), guardião frontend novo com 35 asserções + prova negativa real (gate removido → 3 falhas nomeadas → revertido → verde). Suíte reconciliada em 3 medições: baseline pré-mudança 2964 passed/27 falhas conhecidas de TLS-sandbox → pós-mudança 2980 passed (mesmo sandbox) → 3007 passed fora do sandbox, 0 regressão. **Publicado em produção** (`F10-20260923-02`), Task 3 confirmada e concluída pelo Alex | 2026-09-23 | 0bfec03, d92a829, a0f5888, e7566bf, 157f8fc, 4667bf4 | Published (F10-20260923-02) | [260923-ndy](./quick/260923-ndy-adicionar-botao-executar-na-aba-opcoes-a/) |
