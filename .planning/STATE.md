...
Last activity: 2026-09-09 — quick 260909-waw: **Aba Opções, Fase 1 de 6** mesclada na branch (ADR-027, `mcp_client.py`, cap por usuário, `/api/options/mcp/status`, guardiões reescritos). Plano completo em `docs/PLANO-aba-opcoes.md` (aprovado; decisão revista: Opções ocupa o lugar de "Operador IA" na barra, que vira sub-tela do Portfólio). Próximo: Fase 2 (leitura, setups, troca de aba) após revisão do Alex. Pendências do Alex: `MCP_CLIENT_ID`/`MCP_CLIENT_SECRET` no Railway de staging; confirmar client `boris` em `MCP_CLIENTES`; avalizar o 200+bloqueia do `/status`. Produção ainda em F10-20260908-02 (deploy do #37 pendente de clique).
