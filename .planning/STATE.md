...
Last activity: 2026-09-11 — **auditoria de 2026-09-10 fechada** (21 achados; só ficaram fora A-12 e D-01, que pedem decisão do Alex) e mais um achado ao vivo do painel resolvido: os marcadores de job eram memória do processo e diziam "nunca rodou" depois de cada reinício. Suíte em 2304 pytest + 126 mjs. **Pendente de deploy só-backend** (bump manual): 260911-15a e 260911-axj. Produção em `F10-20260910-04`. Descobertas do executor NÃO corrigidas, aguardando decisão: **D-1** — `/api/options/sell` tem a regressão F10-20260819 ABERTA (`qty=0` vende a posição inteira; `qty="abc"` → 500), só alcançável por chamada direta porque a UI não manda `qty` ali; **D-2** — a classe do A-10 é mais ampla: `scanner.py:350` é o carimbo de "última atualização" VISÍVEL na UI (3 h à frente em produção), e há `date.today()` naive em `options_api.py:36`/`main.py:2477,2502,2703`/`mydata_client.py:202` fazendo aritmética de dias até o vencimento, que alimenta o gate de liquidez.
