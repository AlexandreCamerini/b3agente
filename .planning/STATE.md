...
Last activity: 2026-09-09 — quick 260909-bwm, script master de distribuição iOS (`scripts/distribuir-iphone.sh`). Atualiza `main` (localizando o worktree onde ele está checked out — `git checkout main` não funciona neste worktree), roda a suíte canônica, builda contra produção, sincroniza o projeto nativo, e pergunta o canal (Xcode direto no aparelho × TestFlight) só na hora de instalar. Nunca aponta pra staging nem flipa APNs pra produção sozinho — só lê e avisa. Flag `--no-open` nova em `instalar-iphone.sh` (comportamento antigo preservado sem ela). Não executado ponta a ponta (efeito real fora deste worktree + termina em `read` interativo/GUI do Xcode, que fazem mais sentido rodados pelo Alex); lógica de localização do worktree testada isoladamente e resolve certo.
