...
Last activity: 2026-09-09 — quick 260909-oyu: carimbo de horário das ordens corrigido para BRT (`store.now_str()`/`main.now_str()`/`obslog.ts` usavam `datetime.now()` naive; container Railway em UTC deixava histórico 3h à frente). Backend-only, NÃO deployado — deploy só-backend exige bump manual de `SERVER_BUILD_ID` e clique no Railway (decisão do Alex). Antes disso: quick 260909-dao (App Name "Boris+ Simulador B3" no `TESTFLIGHT.md`); upload do build 10 ao TestFlight ainda sem confirmação.
