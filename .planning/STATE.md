...
status: executing
stopped_at: 'Fase 27 — os CINCO planos executados (2026-09-13): a aba Opções deixou de ser um consultor de tickers avulsos e passou a operar sobre a carteira. O DEFEITO que originou a fase (’os setups não estão sendo gravados’) NÃO era falha de escrita: o setup era gravado no armazém do MCP, mas (a) não existia rota de listar — `list_setups` só era chamado dentro de /leitura/{ticker} e filtrado por ticker, (b) o ticker nascia vazio a cada abertura e (c) o armazém é compartilhado e SEM DONO (ADR-027 Dec. 7), então o conceito ’meus setups’ não existia em lugar nenhum. Agora existe índice por usuário no kv, com namespacing por sha256(uid)[:8] no nome que vai ao serviço — o nome que a pessoa escreveu vive só no índice e na tela. MEDIDO ao vivo antes de construir: o serviço ACEITA nome prefixado e devolve o `name` byte a byte como enviado (checkpoint bloqueante da Task 0, pelo Alex). A leitura técnica (tendência, ADX, HV21/HV63, suporte/resistência, régua de 7 pregões) passou a sair do motor interno determinístico — MESMA fonte do Radar e da Watchlist, custo ZERO de MCP — e o serviço externo ficou só com cadeia/vencimento/payoff, sob clique com o preço escrito no controle. Isso exigiu as Emendas 1 e 2 ao ADR-027, que isolava a aba do núcleo. VERIFICADO NO APP RODANDO (não só em teste): abrir a aba dispara apenas frescor + /api/options/vigias (custo 0); escolher ativo dispara apenas /api/options/tecnico/{ticker}?period=1y; nenhuma ida à rota de custo 3. Universo = carteira provado com carteira de 1 posição contra watchlist de 6. HV em 31,5%/29,9% (unidade correta, não 0,3% nem 3150%). Régua e linha de tendência concordando. Três defeitos de exposição do nome do armazém fechados, um deles achado só porque varremos irmãos: o /grafico imprimia o hash como TÍTULO do gráfico. Gate de dono fechado em /grafico e /desativar via helper único. Suíte 2780 passed, 5 skipped, 3 xfailed, 0 failed + 143/143 .mjs (a fase começou em 2691+137). NÃO ESTÁ NO AR e a sequência importa: 27-01 e 27-02 têm de sair JUNTOS — publicar só o backend quebraria ’Disparos do setup’ e ’Desativar’ no app publicado, porque o front de hoje manda o nome sem prefixo. FORA DE ESCOPO, intacto: ligar a aba à execução de ordens (B3 da Fase 26).'
last_updated: "2026-09-13T15:30:00.000Z"
last_activity: "2026-09-13 (tarde) — **Fase 27 completa em código**, cinco planos, 24 commits na arvore principal (sem worktree). 27-01 indice de vigias por usuario + namespacing; 27-03 motor tecnico interno + regua de 7 pregoes + Emenda 2 do ADR-027; 27-02 universo = carteira, vigias no topo, lastro no cartao, estado vazio com caminho; b0638ef/e220196 gate de dono no /grafico e o nome do armazem parando de vazar para o cliente (3 rotas); 27-04 bloco de leitura interna + ReguaRegime.jsx + unidades.js; 27-05 leitura paga virou clique e CUSTO_DA_ACAO rotulou cada controle. Checkpoint da Task 0 resolvido ao vivo pelo Alex (o servico aceita nome prefixado). Limpeza dos legados feita pelo Alex em 12:03: 3 desativados, abcdef12-teste-fase-27 mantido ate a fase fechar. Verificacao no app rodando confirmou custo zero ao abrir e ao escolher ativo. Suite 2780 + 143 .mjs. Nada publicado, nada empurrado."
progress:
  total_phases: 8
  completed_phases: 5
  total_plans: 38
...
**Current focus:** Phase 27 — Aba Opções sobre a carteira (cinco planos executados; verificação ao vivo e publicação pendentes)
