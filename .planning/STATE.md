...
Last activity: 2026-09-11 — **família do `qty` falsy fechada nas 5 rotas** (`/api/buy`, `/api/sell`, `/api/options/buy`, `/api/options/sell`, `/api/options/lastreada/fechar`), mesma guarda e mesma string; varredura pós-correção não achou sexta. A origem é a regressão F10-20260819, que só a venda de ação tinha corrigido — as outras quatro seguiram abertas por ~3 semanas, e duas foram achadas por executores enquanto corrigiam a anterior. Suíte em 2321 pytest + 126 mjs. **Pendente de deploy só-backend** (bump manual): 260911-cf1 e 260911-ctd; produção em `F10-20260910-04` e `main` já com o bump para `F10-20260911-01` (do lote anterior, ainda não deployado). Aberto, aguardando decisão do Alex: **D-2** (fuso: `scanner.py:350` é o carimbo de "última atualização" VISÍVEL na UI, 3 h à frente; `date.today()` naive em 5 pontos fazendo aritmética de dias até o vencimento, que alimenta o gate de liquidez), **A-12** (bootstrap do 1º admin), **D-01** (recusa por rate limit diz "cota do dia esgotada").
