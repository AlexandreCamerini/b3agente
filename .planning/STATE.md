...
**Current focus:** Milestone v1.6 (Simplificação da aba Opções) — Fase 33 fechada e verificada. Fase 34 (Navegação hub + workspace) EXECUTANDO — 34-01 ✓ (1/4 ondas).

## Current Position

Phase: 34 (Navegação hub + workspace) — **EXECUTANDO, 1/4 ondas**
Plan: 34-01 ✓ | 34-02 pendente | 34-03 pendente | 34-04 pendente (checkpoint humano bloqueante)
...
Last activity: 2026-09-20 — `34-01` completo: `WorkspaceHeader.jsx` (componente novo, props-only, D-05) + 4 chaves de copy (`opcoesVoltarAoHub`, `opcoesAbaAnalisar`, `opcoesAbaComparar`, `opcoesAbaSetupsSalvos`) em `COPY.estudo`/`COPY.operador` + guardião novo `test_opcoes_hub_workspace_ui.mjs` (12 asserções, parte 1: header+copy). Prova negativa real (injeção de `useState`, revertida). Suíte canônica idêntica à baseline da Fase 33: 2923 pytest + 152/153 `.mjs` (1 falha ambiental pré-existente, `test_ios_assets.mjs`). `npx vite build` verde. Commits `7af698d`/`d0a7815`/`e943735`/`03657b1`. `STATE.md`/`ROADMAP.md` confirmados intocados pelo executor (`git diff --stat` vazio).
