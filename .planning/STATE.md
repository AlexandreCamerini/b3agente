...
status: executing
stopped_at: 'Fase 25 — 25-03 executado (2026-09-12, 19:30 -03): Fase 2 do 25-CONTEXT — o catalogo de planos. `plan.py:31-42` era o UNICO bloco de limites 100% literal do app (todos os outros pontos ja tinham env, e tres ja tinham kv + painel + auditoria). Agora `LIMITES_DE_PLANO` e a declaracao unica de cinco pontos de controle, cada linha com `(chave, sufixo de kv, molde de env, tipo, defaults por plano)`, e `limites_do_plano(id)` resolve por memoria → kv → env → default devolvendo valor, ORIGEM, env, chave de kv, default e tipo. Os dois nomes que ja tinham chamador desde a Fase 12 (`max_analyses_per_month`, `max_watchlist`) NAO mudaram — renomear quebraria `can_analyze`/`can_add_ticker`/`can_grow_watchlist_to` sem ganho. QUATRO escolhas de formato, porque as tres fases seguintes consomem exatamente este: (1) `tipo` e coluna e nao inferencia, porque um dos cinco e dinheiro (R$/dia) e inferir do default falharia justo onde importa — `pro` tem None em dois pontos, e None nao tem tipo; (2) o molde de env decide sozinho se a env e por plano (`{PLANO}` no nome) ou global, e e global exatamente nos tres pontos cujo valor HOJE ja e global, reusando `B3_MANAGED_DAILY_QUOTA`, `B3_MCP_COTA_USUARIO_DIA` e `B3_ASSISTENTE_TETO_BRL` — env unica para `max_watchlist` achataria free (10) e pro (sem teto) num numero so, e env nova para os globais criaria um segundo numero para o mesmo teto; (3) sem limite no kv e o `null` do proprio JSON, porque `db.kv_get` ja distingue linha AUSENTE de linha com null — dai o default passado ao kv_get ser um sentinela `_AUSENTE` e nao None, sem o que plano sem limite viraria plano nao configurado no primeiro ida e volta; na env, que so carrega texto, escreve-se `ilimitado`; (4) a precedencia copiou `options_mcp_api._valor_e_origem` com DUAS divergencias declaradas no codigo — la `<= 0` e lixo porque um 0 no teto do servico seria o freio DESLIGADO, aqui 0 e valor legitimo (e como um plano bloqueia um ponto de controle), e la None significa nao achei enquanto aqui None e valor. `set_limite_do_plano` RECUSA lixo com ValueError em vez de clampar com `max(1, int(n))`: la o clamp protege teto fisico compartilhado, aqui 0 e intencao de produto e engolir lixo faria o painel confirmar configuracao que nao foi a pedida; aceita texto porque e o que formulario manda. Funcoes de PRODUTO (D2): `opcoes.criar_setup` declarada em pro e nada em free, `funcoes_do_plano` devolve COPIA e conjunto VAZIO para plano desconhecido (fail-closed), e `FUNCOES_CANDIDATAS` (agente_autonomo, analises_ilimitadas) fica registrada e nao liberada. NINGUEM LE O CATALOGO AINDA — os tres gates seguem lendo o dict estatico de `current_plan`, e `require_criar_setup` segue na permissao do RBAC; ligar e o 25-04, e e la que a pergunta o que acontece quando o kv do plano diz 40 e a env global diz 9 precisa de resposta com nome e sobrenome. DECISAO SOBRE `plan_at_least`, pedida pelo plano: REMOVIDA. Medido antes: zero call sites em server/app e server/tests, o `require_plan()` que a docstring citava como consumidor NUNCA existiu em main.py (e esboco do ADR-013, nao codigo), e `_ORDEM_PLANO` — que ela usava — TEM consumidor real (main.py:1139 ordena planosDisponiveis, travado por test_admin_plano_usuario.py:273). A D2 escolheu conjunto de funcoes em vez de ordem de tier; manter os dois convites garantiria o dia em que um recurso fosse liberado por um mecanismo e outro pelo outro, com regras divergindo sem ninguem notar. O ADR-013 NAO foi tocado (e historia); a remocao e a razao estao datadas na docstring de plan.py, e ha caso de teste que proibe a volta. DESVIO registrado (Rule 3): `server/app/main.py` entrou no escopo por uma linha, `plan.configure_db(_conn)`, no mesmo bloco das outras quatro injecoes — sem ela a camada de kv nasceria morta e a precedencia prometida seria de tres camadas, nao quatro; nao muda comportamento porque nada le o catalogo e sem kv configurado a resolucao devolve os mesmos defaults. GUARDIAO: 31 casos em 8 blocos, com os numeros de hoje escritos POR EXTENSO (30, 10, None, 20, 60, 1.0) em vez de lidos de plan.py — a duplicacao e o ponto, porque guardiao que le o mesmo dicionario que testa acompanha a mudanca em silencio e nao guarda nada; inclui asercao de que o CONJUNTO de chaves nao mudou, para que ponto de controle novo force decisao consciente. RED medido restaurando o plan.py anterior com `git checkout HEAD~1 -- server/app/plan.py`, rodando e restaurando (nada de stash, nada de reset): 31 errors. RESSALVA HONESTA declarada no SUMMARY: deram ERROR e nao FAIL, todos no mesmo ponto (a fixture chama `plan.env_key`, que nao existia), entao o RED prova que a API nao existia e nao que a politica estava errada; os blocos 2 e 3 (frases de recusa byte a byte e os numeros dos gates) sao NAO-REGRESSAO, ja passavam antes e seguem cobertos por test_fase12_cap_analises e test_fase12_cap_watchlist, verdes sem uma linha alterada. LIMITACOES CONHECIDAS, escritas na docstring em vez de viraram surpresa: os overrides GLOBAIS que ja existem (llmDailyQuota do painel em managed, mcpCotaUsuarioDia em options_mcp_api) ficam FORA da cadeia do catalogo, porque plan.py nao pode importar aqueles modulos (ciclo — e por isso que options_mcp_api e fiado por injecao), entao se o admin ja mexeu naqueles dois o catalogo reporta env/default enquanto o servico aplica o do painel, e conciliar e do 25-04; e nao ha como LIMPAR um limite gravado no kv (mesma limitacao que options_mcp_api ja tem, porque db nao expoe delete de chave global) — o 25-05 decide se vira botao voltar ao padrao com funcao nova em db.py. `web/src/plan.js` NAO foi tocado: o espelho do front segue com null nos dois planos por desenho (o limite real chega do endpoint, criterio 6 do ROADMAP da Fase 13, com guardiao .mjs que exige isso). Suite canonica FORA do sandbox: 2621 passed, 5 skipped, 3 xfailed + 132 arquivos .mjs OK (baseline 2590/5/3 + 132 — os +31 sao exatamente os casos novos, e skipped/xfailed/mjs inalterados). Nenhum teste existente de plano/watchlist/quota precisou mudar: os 152 casos do recorte -k "plan or watchlist or quota or metering" passaram sem alteracao. Front nao tocado, entao sem vite build. NAO ESTA NO AR: exige deploy do backend com bump manual de SERVER_BUILD_ID, mas SEM consequencia visivel — nada consome o catalogo ate o 25-04, entao o deploy deste pode ir junto com o do proximo em vez de sozinho. Nada empurrado a origin, nenhum PR, nenhuma publicacao.\n\nAnterior — 25-02 executado (2026-09-12, 15:46 -03): Fase 1 do 25-CONTEXT, decisao D1 do Alex — o papel `owner`, a ancora do dono do produto. Dois fatos MEDIDOS antes: (1) NENHUM papel era de fato irrevogavel — `rbac.revoke_role` era um DELETE cru sem condicao nenhuma, e o que salvava o `role_admin` de sumir era a reconcessao automatica do bootstrap no request seguinte, efeito colateral e nao regra declarada; (2) o papel maximo dependia de `B3_ADMIN_EMAILS` ou, na ausencia dela, da conta mais ANTIGA do banco (`ORDER BY created_at ASC LIMIT 1`), o achado A-12. Agora `OWNER` cai no MESMO ramo de `ROLE_ADMIN` em `permissoes_do_papel`, pela uniao DINAMICA de `GRUPOS`: permissao futura entra sozinha, e ha caso de teste que injeta um grupo ficticio em GRUPOS por monkeypatch para provar — e o unico caso que reprova uma lista literal, que envelheceria no primeiro grupo novo e faria o dono nunca perde funcao virar mentira silenciosa. DUAS CAMADAS, como a D1 pede, cada uma cobrindo o que a outra nao cobre: `revoke_role` levanta `PapelIrrevogavel` (primeira regra do repositorio que declara que um papel nao sai) e a rota responde 403 com a razao — o ramo revogar de POST /api/admin/users/{id}/roles nao tinha verificacao NENHUMA; e `ensure_bootstrap_role` reconcilia o `owner` tambem, alcancando a escrita direta no SQLite que a recusa da rota nao alcanca. Ancora em `B3_OWNER_EMAIL` (default alexandre.camerini@gmail.com), com strip e caixa baixa e SEM o fallback de primeira conta — a ausencia e deliberada e fecha PARCIALMENTE o A-12: `_is_admin_bootstrap` segue intacto para o `role_admin`, porque tres testes dependem dele, e fechar de vez continua sendo decisao separada. Env definida VAZIA significa sem owner, nao use o default. Freio de concessao um degrau acima do que ja existia: `role_admin` distribui qualquer grupo, mas so owner concede owner. PORTAL: `owner` fica FORA de `gruposDisponiveis` (ali viraria um botao de toggle que sempre toma 403) e entra em `papeisIrrevogaveis`, campo novo do backend — a UI o mostra como ESTADO (owner · permanente, span sem onClick), porque some-lo da tela faria o admin abrir a conta do dono e concluir que ela nao tem o papel. Nenhum literal owner escrito no fonte do portal. `agent.py`: a varredura de destinatarios do push de kill-switch enumera GRUPOS, e papel fora de GRUPOS fica invisivel — o dono seria o unico a nao ser avisado de que a execucao automatica da base inteira esta parada, exatamente o incidente que o alerta existe para nao repetir. RED medido antes de cada correcao: backend 12 de 14 vermelhos (`AttributeError: module app.rbac has no attribute OWNER`, `assert set() == {futuro.mandar}`, e o 400 Papel desconhecido: owner); portal 7 de 19 assercoes, contra os fontes de HEAD copiados para diretorio temporario (nada foi revertido nem stashed na arvore). Passam nos DOIS estados a nao-regressao (revogar outro papel pela rota continua funcionando) e a sanidade da uniao; os tres casos de sem e-mail correspondente ficaram vermelhos por AttributeError e nao por comportamento — antes do papel existir eram verdadeiros por vacuidade, e a ressalva esta escrita no SUMMARY em vez de omitida. DESVIO registrado: guardiao estatico novo do portal (web/tests/test_admin_owner_ui.mjs, precedente do test_admin_plano_ui.mjs), que o plano nao previa mas o criterio de aceite cobrava — e no proprio ensaio de RED dele apareceu uma assercao de ausencia que passava por VACUIDADE sobre string vazia, corrigida (o RED subiu de 6 para 7 falhas). NENHUM guardiao do ADR-013 foi afrouxado nem tocado: as 9 permissoes cravadas em test_adr013_rbac.py seguem exatas, porque o owner nao acrescenta permissao nenhuma, deriva as mesmas. Suite canonica FORA do sandbox: 2590 passed, 5 skipped, 3 xfailed + 132 arquivos .mjs OK, exit 0 (baseline 2576/5/3 + 131 — os +14 e o +1 sao exatamente os casos novos). `cd web-admin && npx vite build` verde. NAO ESTA NO AR, e sao DUAS publicacoes em ordem: deploy do backend com bump manual de SERVER_BUILD_ID e so DEPOIS `publicar-admin.sh` — portal novo contra backend velho leria um campo inexistente. No primeiro request pos-deploy a conta alexandre.camerini@gmail.com recebe `owner` sozinha; se o e-mail de producao for outro, basta definir `B3_OWNER_EMAIL` no Railway, e sem isso NAO havera owner nenhum, por desenho. O `owner` ainda NAO pula o cap comercial: a D3 e a Fase 3. Nada empurrado a origin, nenhum PR, nenhuma publicacao.\n\nAnterior — 25-01 executado (2026-09-12, 14:55 -03): conserto do CONTADOR que o gate comercial le, antes de a Fase 2 torna-lo configuravel — dar controle fino sobre uma medicao errada seria pior que nao ter controle. Defeito 1 FECHADO: `POST /api/analytics/events` chamava `metering.consume` sem `month_section` e caia no default `MONTH_SECTION = aiUsageMonth`, o MESMO ledger que `plan.can_analyze` le pelo `month_used`; como o custo ali e o tamanho do LOTE (o cliente envia ate 50 por vez), cada sessao de uso do app descontava dezenas de analises de quem nao pediu nenhuma. A telemetria ganhou `analyticsEventsMonth`; os DOIS nomes antigos (`analyticsEvents`, `analyticsEventsGlobal`) mantiveram o valor byte a byte, porque ha contador gravado no kv sob eles. MEDICAO da Task 2 (somente leitura, nada escrito): das 7.273 contas do banco local, TRES tinham ledger mensal, e as tres com `count` EXATAMENTE igual ao numero de eventos de telemetria ingeridos no mes (4.167/4.167, 2.348/2.348, 25/25) — 100% do contador era telemetria, 0% era analise; DUAS estao acima do limite de 30. Ressalva declarada no SUMMARY: esse banco e de TESTE (7.146 contas @teste.com), nao producao — o que transfere e a estrutura, nao o numero. DEFEITO 2 NAO FOI ATIVADO, POR DESENHO: ligar o gate mensal em `/api/scan/deep`, `/api/carteira-stopalvo` e `/api/assistente` hoje barraria conta pelo RESIDUO do defeito 1 (o contador ja gravado so zera na virada de mes), nao por uso — e o plano manda PARAR e relatar nesse caso. A decisao e do Alex, com tres opcoes e o preco de cada uma no SUMMARY; a recomendacao e ativar junto com os limites configuraveis da Fase 2, como o proprio 25-CONTEXT ja previa. O estado pendente ficou CODIFICADO na suite como `xfail(strict=True)`: no dia da ativacao os tres casos viram XPASS e FALHAM, obrigando quem ativar a tirar a marca. DIAGNOSTICO CORRIGIDO no caminho, e vale para a Fase 3: as tres rotas JA CONTAM no ledger que o plano le (o `consume` de `_ai_apply_managed` usa o `MONTH_SECTION` default) — o que falta e o GATE, nao a contagem; elas gastam acima do cap e registram o excesso. Guardiao de `month_section` deixou de varrer so `options_mcp_api.py` e passou a varrer `server/app/*.py` inteiro, exigindo que o balde seja ESCRITO (nao um literal unico: cada dominio tem o seu), com sanidade sintetica e PROVA POR INJECAO — um `consume` pelado postico em `main.py` foi reprovado apontando a linha, e o arquivo restaurado de copia. `_ai_apply_managed` passou a declarar `month_section=metering.MONTH_SECTION`: o balde nao muda, o que muda e ter sido escolhido. `metering.snapshot` ganhou `month_section` — o `section` seguia para o dia e o mes vinha sempre de `aiUsageMonth`, entao o snapshot da telemetria misturava `used` de um balde com `monthUsed` de outro. RED medido antes de cada correcao: `assert 50 == 0` na telemetria; `/api/scan/deep` respondendo 200 com o mes em 100/30 e as outras duas CHAMANDO a IA com o mes estourado; `TypeError` no `snapshot`; injecao reprovada no guardiao. ZERO mudanca de politica: `server/app/plan.py` fora do diff dos tres commits, nenhum limite alterado. Suite canonica FORA do sandbox: 2576 passed, 5 skipped, 3 xfailed + 131 .mjs, exit 0 (baseline 2565/5 + 131 — os +11 e os 3 xfailed sao exatamente os casos novos). NAO ESTA NO AR: e mudanca de backend e exige deploy com bump manual de `SERVER_BUILD_ID`; ate la producao continua descontando telemetria da cota de analise. Nada empurrado a origin, nenhum PR, nenhuma publicacao.\n\nAnterior — 24-17 executado (2026-09-12, 12:35 -03): pedido do Alex do próprio dia — mudar o plano de uma conta pelo portal admin. Até aqui `pro` era, nas palavras do próprio `server/app/plan.py`, "estado alcançável só por atribuição manual", e a única porta era editar o SQLite do container (`scripts/plano-da-conta.sh`, criado nesta mesma sessão para promover a conta do Alex). Serve para a conta do dono; não serve para o dia em que houver assinante. `POST /api/admin/users/{user_id}/plan` espelha a rota de papéis que já existia ao lado: gate `usuarios.gerenciar`, 404 para conta inexistente, 400 nomeando os aceitos para plano fora de `plan.PLANOS_POR_ID` (o BACKEND recusa, não só a UI — a tela não é o único cliente possível) e `audit.record` com entidade `user_plan`, o valor anterior e o novo. `user_plan` entrou em `rbac.ENTIDADES_POR_PERMISSAO["usuarios.gerenciar"]`: sem isso o evento é gravado e `entidades_visiveis` o filtra para fora de todo filtro, inclusive o de quem acabou de produzi-lo. A lista de planos tem FONTE ÚNICA: sai de `PLANOS_POR_ID` na ordem de `_ORDEM_PLANO`, viaja como `planosDisponiveis` em `GET /api/admin/users` (o par comercial de `gruposDisponiveis`) e a UI só renderiza — nenhum id de plano escrito no `main.py` nem no portal, inclusive o fallback "free" da exibição, que virou travessão. ISTO REVERTE UMA DECISÃO REGISTRADA, e a reversão ficou registrada em vez de apagada: a docstring de `POST .../roles` dizia "Sem override de plano nesta rodada (decisão do Alex, ADR-013)" e foi ATUALIZADA com a data; o comentário de topo do card no portal, idem; e mais DUAS notas fora do escopo do plano passariam a mentir (`db.set_user_plan` e `plan.current_plan`), então também foram datadas — registro de decisão que mente é pior que registro ausente. DECISÃO QUE NÃO É PARA MELHORAR: mudar o PRÓPRIO plano continua permitido e auditado, com teste próprio carregando a razão — quem tem `usuarios.gerenciar` já concede a si mesmo qualquer papel de governança pela rota irmã, e o registro com o nome de quem clicou é a mitigação que o ADR-013 escolheu para essa classe; um freio só aqui seria regra inventada e assimétrica. RED medido antes de cada correção: backend `21 failed, 2 passed` de 23; front `12 falha(s)` de 27. Um caso passava por VACUIDADE (o 404 de conta inexistente, porque o catch-all `_api_inexistente` também responde 404) e foi endurecido para exigir o texto da rota. Suíte canônica 2565 passed, 5 skipped + 131 .mjs, exit 0 (baseline 2542/5 + 130 — os +23 e o +1 são exatamente os novos); `cd web-admin && npx vite build` verde; allowlist pública do ADR-013 NÃO cresceu (a rota é gated). NÃO ESTÁ NO AR, e são DUAS publicações em ordem: deploy do backend (com bump manual de `SERVER_BUILD_ID`) e só depois `publicar-admin.sh` — o portal publicado antes do backend chamaria rota inexistente. Nada empurrado a origin, nenhum PR.\n\nAnterior — 24-16 executado (2026-09-12, 01:50 -03): correção de uma promessa não cumprida, achada a partir de um bloqueio real na conta do Alex. Ele bateu as 30 análises/mês do PLAN_FREE e, ao configurar a própria chave de LLM, continuou barrado — porque `_gate_analise` consultava `plan.can_analyze(metering.month_used(...))` ANTES de qualquer verificação de BYOK, e `month_used` lê um ledger cujo único ponto de escrita (`metering.consume`) só roda no ramo GERENCIADO de `_ai_apply_managed`. Ou seja: quem traz a própria chave era barrado por consumo de um recurso que não vai usar, pagando do bolso pelo que não conseguia rodar — e o código já prometia o contrário em dois lugares (o comentário `# BYOK utilizável → sem cota` e o texto do 402, que fala de "análises do seu plano" para quem não usa a chave do plano). A precedência ganhou um degrau ACIMA dos dois gates: BYOK → PLANO (mensal) → METERING (diário). Não é mudança de política comercial: muda QUANDO o gate mensal é consultado, nunca O QUE ele mede — `plan.py` intacto byte a byte, `metering.py` intacto, nenhum contador novo, nenhuma seção de kv nova. `llm.resolve_key(config)` é a MESMA expressão que `_ai_apply_managed` já usava (duas definições de "tem chave" divergem na primeira manutenção, e uma delas decidiria dinheiro), e o ramo novo CHAMA `_ai_apply_managed` em vez de copiar o retorno dele. 10 casos novos em `server/tests/test_plan_gate_byok.py`, provados RED contra o código anterior (`4 failed, 6 passed`): falharam os casos 1/1b/1c (a correção) e o 5 — este último porque só consegue medir o ledger DEPOIS de atravessar o gate com chave própria, ou seja, é sub-asserto do caminho novo e não prova independente de não-regressão; o desvio da previsão do plano está registrado no SUMMARY e na docstring do teste. Passam nos DOIS estados os casos 2 (sem BYOK continua barrando com a mensagem exata — o guardião do cap comercial, tão importante quanto o 1), 2b, 3, 4 (pro, com e sem chave) e 5b (anônimo segue fail-closed). Vale para as TRÊS rotas que chamam `_gate_analise` (`/api/analyze`, `/api/technical/analyze` e a compilação de setups da aba Opções); nenhuma dependia da ordem antiga, e os guardiões estáticos pré-existentes seguem verdes. Suíte canônica 2542 passed, 5 skipped + 130 .mjs (baseline 2532/5 + 130 — +10 são exatamente os novos). NÃO ESTÁ NO AR: é mudança de backend e exige deploy (com bump manual de SERVER_BUILD_ID); até lá a conta do Alex continua barrada em produção. Nada empurrado a origin, nenhum PR, nenhuma publicação.\n\nAnterior — 24-15 executado (2026-09-12, 00:35 -03): os três tetos da aba Opções (`B3_MCP_COTA_USUARIO_DIA` 60, `B3_MCP_RATE_MIN` 20, `B3_MCP_COTA_GLOBAL_DIA` 1800) saíram de env-só-do-Railway e passaram a ser configuráveis pelo portal admin, sem deploy. Pedido do Alex em 2026-09-11: o que esses três freiam é o contrato do serviço — 2.000 `tools/call` por dia para TODA a base somada — e a Fase 24 acabou de multiplicar o consumo por sessão (`/possibilidades` custa até 13), então o ajuste precisa ser de minutos quando o consumo real aparecer no `/observabilidade`. Precedência memória → kv → env → default, no padrão de `brapi_budget.spot_intervalo_s`; a env CONTINUA sendo camada e continua lida a cada leitura (é como o Railway troca um teto sem publicar código, e é o que 39 testes de cap exercitam com `monkeypatch.setenv`) — o cache em memória é só do valor vindo do kv. `limites()` publica a ORIGEM de cada número (`kv`/`env`/`default`), sem a qual o admin muda pelo painel, a env continua diferente e ninguém sabe qual manda. `GET`/`POST /api/obs/opcoes/cota` sob `fontes_dados.configurar` (a MESMA do orçamento brapi — é teto de fonte de dados, não governança de IA), com prévia, validação tudo-ou-nada antes de aplicar e um `audit.record("mcp_cota")` POR CAMPO alterado. Valor acima de 2.000/dia é ACEITO com aviso: recusar fingiria que o Boris manda no teto do serviço, calar deixaria o admin achar que subiu um teto que não subiu. Card no portal dentro da aba "Fontes de dados" (nenhuma aba nova, VIEWS intacto), com origem traduzida, consumo global de hoje e a frase do teto compartilhado. Suíte canônica 2532 passed, 5 skipped + 130 .mjs, exit 0 (baseline 2511/5 + 129 — +21 pytest são exatamente os novos, +1 .mjs é o guardião do card); `cd web-admin && npx vite build` verde. Nada empurrado a origin, nenhum PR, nenhuma publicação: o deploy do backend e o `publicar-admin.sh` são etapa posterior, já combinada com o Alex.'
last_updated: "2026-09-12T22:30:00.000Z"
last_activity: "2026-09-12 (noite) — **25-03** executado na arvore principal (sem worktree): o catalogo de planos. `plan.py` era o unico bloco de limites 100% literal do app; virou uma tupla declarativa de cinco pontos de controle que resolve por memoria → kv → env → default e diz a ORIGEM de cada valor, mais funcoes de produto por plano (D2, `opcoes.criar_setup` em pro). Os dois nomes com chamador desde a Fase 12 nao mudaram. O formato foi escolhido pensando em quem consome: o tipo e coluna (um dos cinco e dinheiro), o molde de env decide sozinho se e por plano ou global — e e global exatamente onde o valor de hoje ja e global, reusando a env que ja controla o teto —, e sem limite no kv e o null do proprio JSON, com sentinela `_AUSENTE` para nao achei, senao plano sem limite viraria plano nao configurado no primeiro ida e volta. NADA MUDA DE COMPORTAMENTO: os tres gates seguem lendo o dict estatico; ligar e o 25-04. `plan_at_least` REMOVIDA — orfa desde que nasceu e apontando para um `require_plan()` que nunca existiu; a D2 escolheu conjunto de funcoes, nao ordem de tier, e manter os dois garantiria o dia da divergencia. `_ORDEM_PLANO` fica (main.py ordena planosDisponiveis por ela). Desvio Rule 3: uma linha em main.py (`plan.configure_db(_conn)`), sem a qual a camada de kv nasceria morta. 31 casos novos com os numeros de hoje escritos por extenso; RED de 31 errors contra o plan.py anterior, com a ressalva honesta de que foi ERROR e nao FAIL (a API nao existia) e de que os blocos de frase/numero sao nao-regressao. 2621 passed, 5 skipped, 3 xfailed + 132 .mjs, sem regressao. NAO esta no ar, mas sem consequencia visivel ate o 25-04 — o deploy pode ir junto com o do proximo.\n\nAnterior (2026-09-12, tarde) — **25-02** executado na arvore principal (sem worktree): o papel `owner`. O dono do produto deixou de depender de ordem de criacao de conta e de alguem lembrar de configurar uma lista de e-mails. Permissoes pela uniao DINAMICA de GRUPOS, o mesmo mecanismo do `role_admin` — permissao futura entra sozinha, com caso de teste que injeta um grupo ficticio por monkeypatch para provar. Irrevogavel nas DUAS camadas da D1: `revoke_role` levanta `PapelIrrevogavel` e a rota responde 403 (o ramo revogar nao tinha verificacao nenhuma), e `ensure_bootstrap_role` reconcede se o papel sumir por escrita direta no banco. Ancora em `B3_OWNER_EMAIL` SEM fallback de primeira conta: fecha parcialmente o A-12, e o `_is_admin_bootstrap` do `role_admin` segue intacto porque tres testes dependem dele. So owner concede owner. No portal o papel aparece como ESTADO, nunca como toggle — dentro de `gruposDisponiveis` seria um botao que sempre toma 403, e fora da tela faria o admin achar que a conta do dono nao tem o papel. O owner entrou na varredura do push de kill-switch. RED medido: 12 de 14 no backend, 7 de 19 no guardiao do portal. 2590 passed, 5 skipped, 3 xfailed + 132 .mjs, exit 0; build do web-admin verde. NAO esta no ar: deploy do backend (bump manual de SERVER_BUILD_ID) e so depois publicar-admin.sh.\n\nAnterior (2026-09-12, tarde) — **25-01** executado na arvore principal (sem worktree): o contador mensal que o plano comercial le volta a medir analise. A rota de telemetria (`POST /api/analytics/events`) descontava o LOTE inteiro da cota de 30 analises/mes do free, porque chamava `metering.consume` sem `month_section` e caia no default `aiUsageMonth` — o ledger do gate. Agora tem balde proprio (`analyticsEventsMonth`), com os dois nomes antigos preservados byte a byte. MEDIDO antes de qualquer decisao: as tres contas com ledger no banco local tinham `count` igual ao numero exato de eventos de telemetria — 100% do contador era telemetria. Por isso a ATIVACAO do gate mensal nas tres rotas orfas (`/api/scan/deep`, `/api/carteira-stopalvo`, `/api/assistente`) NAO foi feita: ligar hoje barraria conta pelo residuo do defeito, e a decisao voltou para o Alex — tres opcoes no SUMMARY, recomendacao de ativar junto com a Fase 2. Os tres casos ficaram como `xfail(strict=True)`, que FALHAM no dia da ativacao. Guardiao de `month_section` passou a varrer todo o `server/app/`, com prova por injecao. `metering.snapshot` parou de misturar o dia de um dominio com o mes de outro. `plan.py` intacto, nenhum limite alterado. 2576 passed, 5 skipped, 3 xfailed + 131 .mjs, exit 0. NAO esta no ar: exige deploy de backend com bump manual de SERVER_BUILD_ID.\n\nAnterior (2026-09-12, meio-dia) — 24-17 executado na árvore principal (sem worktree): o plano de uma conta passa a mudar pelo portal admin, sem tocar no banco. Pedido do Alex do próprio dia, depois de a promoção dele ter saído por `scripts/plano-da-conta.sh` direto no SQLite do container. `POST /api/admin/users/{user_id}/plan` sob `usuarios.gerenciar`, espelhando a rota de papéis: 404, 400 nomeando os aceitos, `audit.record` com entidade `user_plan`. `user_plan` entrou em `ENTIDADES_POR_PERMISSAO` (sem isso o evento some do filtro de auditoria de quem o produziu). A lista de planos vem do backend (`planosDisponiveis`, de `plan.PLANOS_POR_ID`): nenhum id de plano escrito no portal. Reverte decisão registrada — as QUATRO notas de sem override de plano foram atualizadas com a data, nunca apagadas. Mudar o próprio plano segue permitido e auditado, por decisão, com teste que carrega a razão. RED medido: backend 21 de 23, front 12 de 27; um caso passava por vacuidade (catch-all de 404) e foi endurecido. 2565 passed, 5 skipped + 131 .mjs; build do web-admin verde. NÃO está no ar: exige deploy de backend (bump manual de SERVER_BUILD_ID) e, só depois, `publicar-admin.sh`.\n\nAnterior (2026-09-12, madrugada) — 24-16 executado na árvore principal (sem worktree): BYOK passa na frente do gate MENSAL do plano em `_gate_analise`. Achado a partir de um bloqueio real na conta do Alex (30/30 análises do PLAN_FREE): configurar a própria chave não destravava, porque o gate mensal rodava antes de qualquer verificação de BYOK e usa um contador que só mede o consumo da chave do SERVIDOR. Precedência nova: BYOK → PLANO (mensal) → METERING (diário). `plan.py` e `metering.py` intactos — muda QUANDO o gate é consultado, não O QUE ele mede. 10 casos novos em `test_plan_gate_byok.py`, RED medido (`4 failed, 6 passed`); o caso 2 (sem BYOK continua barrando) é o guardião do cap comercial. 2542 passed, 5 skipped + 130 .mjs. Não está no ar: exige deploy de backend com bump manual de SERVER_BUILD_ID.\n\nAnterior (2026-09-12, madrugada) — 24-15 executado na árvore principal (sem worktree): os três tetos da aba Opções deixaram de ser só env do Railway. Pedido do Alex de 2026-09-11. Precedência memória → kv → env → default (padrão do `brapi_budget`), com a env preservada como camada e lida a cada leitura; `limites()` publica a ORIGEM de cada número; `GET`/`POST /api/obs/opcoes/cota` com prévia, tudo-ou-nada e auditoria por campo; card no portal dentro da aba Fontes de dados, com consumo do dia e a ressalva de que o teto de 2.000/dia é do SERVIÇO. 2532 passed, 5 skipped + 130 .mjs; build do web-admin verde. Publicação e deploy são etapa posterior.\n\nAnterior (2026-09-12, madrugada) — 24-14 executado na árvore principal (sem worktree): terceiro achado ao vivo da mesma leva, agora sobre o ENSAIO do setup. `disparos: 0` com `pregoes_avaliaveis: 31` num setup cuja condição de média nunca teve valor — números certos que se leem como teste feito. O backend passou a devolver `ensaio` ao lado do `backtest` verbatim e a tela mostra, ACIMA dos números, quais condições nunca puderam ser verificadas; o botão de gravar continua existindo. 2511 passed, 5 skipped + 129 .mjs, exit 0.\n\nAnterior (2026-09-11, noite) — 24-12 executado na árvore principal (sem worktree): segundo achado ao vivo do mesmo dia, agora sobre o CARIMBO do dado. A aba dizia \"dado em dia\" numa sexta sobre cotação de terça: `negociacao_b3` com 49,58 h de idade contra um SLA de 96 h é `em_dia` pelo contrato do fornecedor, e mesmo assim faltavam DOIS pregões na base. Decisão: medir em vez de herdar. `_atraso_em_pregoes(data, _hoje=None)` conta os pregões ESTRITAMENTE entre o dado e hoje usando `pregao.is_trading_day` — fonte única do calendário (Carnaval, Corpus Christi, exceções por ofício, `B3_FERIADOS_EXTRA`), com guardião que proíbe `weekday()` dentro do helper. O dia corrente NUNCA conta (o COTAHIST de um pregão só sai depois do fechamento; contar hoje faria o app acusar atraso toda manhã sobre dado que ainda não poderia existir) e data ausente, torta ou futura é `pregoes: None`, nunca 0 — 0 afirmaria \"está no último pregão fechado\". `atraso` entra no envelope de `/status` e `/leitura` derivado do MESMO valor que já vai em `pregao` (variável extraída, não recalculada), sem tool nova: custos declarados 1 e 3 intactos. No front o chip ganha a QUARTA variante, com PRECEDÊNCIA sobre \"dado em dia\" (guarda explícita no ramo + ordem no fonte, as duas travadas por guardião com sanidade que reprova a ordem invertida); quando o frescor também bloqueia, as duas informações SOMAM (\"dado atrasado (50 h) · 2 pregões atrás\") porque medem coisas diferentes. `opcoesAtrasoPregoes`/`opcoesAtrasoAjuda` nas duas vozes, com a ajuda aparecendo só quando há contagem na tela. NADA passou a ser bloqueado: quem barra veredito e criação de setup continua sendo o frescor do serviço (ADR-027, Decisão 8), com asserção explícita disso. Regra de ouro cumprida: os 16 testes de backend vistos VERMELHOS antes (`AttributeError: ... no attribute '_atraso_em_pregoes'` ×14, `KeyError: 'atraso'` ×2) e as asserções do bloco 12 do guardião de front também (`14 FALHA(S)` de 17 rodadas contra o fonte de antes; o bloco consolidado tem 19). Todo teste novo tem relógio FIXO por `_hoje` — nenhum muda de resultado conforme o dia em que a suíte roda. Suíte canônica: 2473 passed, 5 skipped + 129 .mjs, exit 0 (baseline 2457/5). `npx vite build` verde, com `web/dist` restaurado depois a partir de `server/web_dist` — outra sessão publicou o front na mesma árvore durante a execução (`F10-20260911-06`), e o build teria contaminado o artefato dela. Nada empurrado a origin, nenhum PR.
...
Last session: 2026-09-12T22:30:00Z
Stopped at: Fase 25, plano 03 concluído — o catálogo de planos (Fase 2 do `25-CONTEXT.md`). Estrutura criada e provada inerte: sem configuração, `free` e `pro` decidem exatamente como antes. Quem passa a LER o catálogo é o 25-04. Deploy do backend é etapa posterior, sem urgência — nada consome o catálogo ainda.
