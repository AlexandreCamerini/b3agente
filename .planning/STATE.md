...
Last activity: 2026-09-10 — **verificação ao vivo no iPhone** (build F10-20260910-01, instalado da branch contra produção): a aba Opções mostra "serviço de opções não configurado", o que prova DUAS coisas — (a) `MCP_CLIENT_ID`/`MCP_CLIENT_SECRET` ainda NÃO estão no Railway de produção (a rota `/status` da Fase 1 existe e responde 503); (b) o conserto 260910-d57 funciona: antes dele o 404 da rota de leitura (Fase 2 não deployada) mascararia essa mensagem acionável. Carimbo do front bumpado para F10-20260910-01 — estava parado em -20260908-02 porque o bump da Fase 1 foi só do backend, e o app mostrava 08/09 rodando código novo. Fases 1 (em `main`, no ar) e 2 (PR #39, aberto) prontas. Pendências do Alex: segredo do MCP no Railway; mesclar #39 e deployar. Próximo: Fase 3 (cadeia, catálogo, proposta e possibilidades por vencimento).
