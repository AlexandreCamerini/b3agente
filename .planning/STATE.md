...
**Current focus:** Milestone v1.7 (Confiabilidade explicativa da aba Opções) — Fase 36 (Motor de Payoff Genérico) EXECUTANDO — 36-01 ✓ (1/2 ondas). Próximo: onda 2 (36-02, domínio/segmentos + caso golden).

## Current Position

Phase: 36 (Motor de Payoff Genérico) — **EXECUTANDO, 1/2 ondas**
Plan: 36-01 ✓ | 36-02 pendente
Status: `/gsd-execute-phase 36` rodando nesta sessão (2026-09-21), sem worktree (`workflow.use_worktrees=false`), executor sequencial por onda. Cada executor foi instruído a NÃO tocar STATE.md/ROADMAP.md — o orquestrador edita à mão após cada onda.
Last activity: 2026-09-21 — `36-01` completo: `server/app/opcoes_payoff.py` ganhou `vencimento` opcional/opaco por perna (D-03, degradação honesta quando divergem — sem curva, sem ganho/perda máxima), guarda de entrada degenerada com `ValueError` (D-04.1, 3 condições obrigatórias: custo zero E curva inteira zero E inclinação da cauda zero), e a correção do breakeven espúrio em S=0 (D-04.2). **Verificado por mim de novo, direto no módulo modificado, não só confiando no SUMMARY**: os 6 casos batem exatos — golden `[49.42]`, CALL prêmio zero `[50.0]` (era `[0.0, 50.0]`), ratio spread `[50.0, 60.0]` (era `[0.0, 50.0, 60.0]`), box travado `[]` (era `[0.0, 50.0, 55.0]`), entrada degenerada levanta `ValueError` com mensagem clara, straddle sem regressão `[48.0, 52.0]`. 48 testes nomeados no arquivo (28 pré-existentes + 20 novos). Prova negativa real executada (reverteu a condição, viu os 3 casos falharem nomeados, restaurou, viu verde de novo). **Achado de ambiente, não do produto**: `bash scripts/executar.sh --testes` travou de novo neste host (mesma trava de `lsof`/`ps` já documentada na Fase 35) — contornado rodando as duas suítes manualmente: 2943 pytest (baseline 2923 + 20 novos, exato) + 154/154 `.mjs`. `git diff --stat -- web/` vazio. Commits `30e6d1c`/`736a2ce`/`0fbc52d`/`1410599`.

**Contexto do planejamento (sessão anterior, mesmo dia):** `/gsd-discuss-phase 36` → `gsd-pattern-mapper` → `gsd-planner` opus (2 planos) → `gsd-plan-checker` (VERIFICATION PASSED, 0 blockers). Achado central: `server/app/opcoes_payoff.py` já era um motor genérico em produção desde a Fase 15 — esta fase ESTENDE (D-01), nunca recria.

**Próximo passo:** `/gsd-execute-phase 36` continua — Onda 2 (36-02, `dominio_da_curva()`/`segmentos_da_curva()` + caso golden nomeado).
